...
`client.convex` is the frame's own authenticated Convex client, a `ConvexReactClient` from `convex/react`, and `client.api` holds typed references to the plugin doors, generated from the app into `convex-api.d.ts` (`bonobo-plugin-sdk/convex-api`). The SDK owns the client: it opens it on init, keeps it authenticated, and closes it on `pagehide`. A plugin never builds a second one. `react` is a peer dependency of the SDK, and a plugin that uses the hooks also needs the `convex` package for the `convex/react` import.

**A plugin that imports `convex/react` itself must end up with the same `convex` install as the SDK.** The SDK builds the client with its own copy, and the plugin's hooks read the client that the plugin's `ConvexProvider` was handed. Two different `convex` versions in one bundle means the hooks drive a client built by another copy of the library, which type-checks and then behaves in ways nothing here promises. The SDK declares `convex` as a normal dependency, so nothing stops that: keep the plugin's `convex` range equal to the SDK's (`^1.42.2`) and check the plugin's lockfile resolves exactly one `convex` before releasing. A plugin that never imports `convex` itself — Gallery, Video Player and Council today — cannot hit this.
