...
While a reader binding is attached, private-scope membership changes update the file's readers. A real manual sharing change detaches only that node's binding and keeps its remaining grants. Future scope changes then leave those readers alone. Failed or no-op sharing and metadata edits keep the binding. Explicit plugin access may attach it again with manage permission. A foreign installation binding refuses until it drains or is detached. Uninstall removes bindings but preserves file grants. Reinstall and repeated ensure do not restore old sharing. The organization owner still reads everything.
