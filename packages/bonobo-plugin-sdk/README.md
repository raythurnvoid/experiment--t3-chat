...
- `idempotencyKey` names one upload run (the meeting); `targetKey` names one file in it. The pair is the file's identity, so a later run may reuse a target key for a different file. Replays with the same body answer the same target; the same key pair with a different body answers `409`.
- There is nothing to book first. `create-target` charges the declared `size` to the workspace's 10 GiB `plugin_service_storage_bytes` quota right away, and answers `403` when that would cross the ceiling. The counter only grows: deleting a stored file gives nothing back, and an upload you abandon still costs its declared size. At most 16 targets per upload run.
...
- Upload with a signed PUT to `uploadUrl` (send exactly the returned `headers`), then poll `finalize` until `state` is `"committed"`. `remint` when the 15-minute URL expired mid-retry. `finalize` answering `"released"` means the upload window is gone for good — retry under a new `targetKey`. The signed PUT does not bind the object's length: if you store more than you declared, `finalize` charges the difference instead of refusing it.
- A target you create is a real, empty file in the workspace from that moment. If your service crashes before the PUT, the empty file simply stays there and a member can delete it — the host never sweeps it for you. Cancel it yourself with `delete` when you know the upload will not happen.
- `delete` removes the files a `targetKey` stored, and is also how you cancel an upload that will never finish. The Council delete does not use it — it archives its whole folder with `archive-destination` below. It works days later under a NEW grant sealed to the same destination, and it only sees targets whose stored path is inside the presenting grant's prefix. A committed file answers `"deleting"` immediately (file gone from the workspace, R2 deletion enqueued) and `"deleted"` once R2 confirmed the object is gone. An unfinished upload has no confirmed object to wait for, so it answers `"deleted"` straight away. Nothing is ever refunded. A read-only file answers `409` and nothing is deleted; an unknown key answers `404`. Replays keep answering.
...
- Error statuses: `400` invalid input, `401` dead grant/installation/membership, `403` missing scope, wrong phase, destination fence, or a full quota, `404` unknown target (including another workspace's), `409` idempotency conflicts and replay-after-release.
