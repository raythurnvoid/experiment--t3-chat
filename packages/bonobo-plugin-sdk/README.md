...
The client's `data`, `members` and `scopes` APIs run as reactive queries and mutations on the frame's own Convex client — not over the bridge, and not on the UI token. To authenticate it, the SDK uses the plugin-session JWT the host delivers with the session token (in `bonobo:init` and in every `bonobo:token`); it identifies the viewing member with plugin-scoped permissions and expires with the token. When Convex needs a fresh JWT, the SDK refreshes the session through the host and hands over the JWT that comes back. A host that sends no JWT is covered by exchanging the token at `POST <apiOrigin>/plugins-ui/session-jwt` (same-origin, so no CORS); there a `401` gets one host token refresh and one re-exchange before the client goes unauthenticated. A one-second wake poll notices a wall-clock gap of 30 seconds and calls `setAuth` again, so Convex pauses before an old session can reconnect while the host re-mints it. The Convex functions load the session on every call, so revoking it (uninstall, disable, upgrade, or a second load of the same frame) turns every live subscription into a `null` death — a signed-valid JWT does not outlive its session. The UI token still never carries the plugin-data write scope; frame writes go through member-attributed `user*` mutations instead.
...
### Using the Convex client directly

`client.convex` is the frame's own authenticated `ConvexClient`, the same instance the `data`, `members` and `scopes` calls above run on, and `client.api` holds typed references to the plugin doors, generated from the app into `convex-api.d.ts` (`bonobo-plugin-sdk/convex-api`). Use them for a call the wrappers do not offer, for example a one-document read that should not spend a subscription slot. TypeScript checks the arguments and the delivered value:

```ts
const unsubscribe = client.convex.onUpdate(
	client.api.plugins_data.watch_documents,
	{ collection: "cursors", keyPrefix: `me:${userId}`, limit: 1 },
	(result) => {
		// `result` is `{ docs, truncated }`, or `null` when the door refused the read.
	},
	(error) => {
		// The query failed. Nothing is retried for you.
	},
);
...
What a direct call gives up compared with the wrappers:

- Nothing is counted. The 16 subscription slots, the 6 intervals per window and the 100 server subscriptions are counters inside this SDK. A direct subscription is invisible to them, and the server enforces no per-frame cap, so a page that opens subscriptions in a loop is limited only by the browser. Design within the wrappers' budget anyway; the counters describe what one well-behaved frame should spend.
- No death reasons. A wrapper turns the door's `null` into `{ reason, message }`. Here you read the door's own answer: `null` for a refused read (the plugin was uninstalled or disabled, the capability is not accepted, the session is gone), the door's `_nay` for a refused write, and a rejected promise or an `onError` call for a transport failure, with no retry and no `unavailable` fallback.
- The identity is the plugin-session JWT, and only the plugin doors resolve it to a member. Every other function of the app sees no user behind it: a query that needs a member answers `null` and a mutation that needs one answers `Unauthenticated`. The app's own tests pin this for `users:get_anagraphic` and `ai_chat:thread_create`. `client.api` lists everything the JWT can call as a member.
- The client closes on `pagehide`. A subscription opened directly ends with it.

The doors are the 13 functions the wrappers use: the `user_*` writes, `user_manage_scope`, the `watch_*` reads, `resolve_member_display` and `list_members`. Their argument and result types are the app's own. A user id is `BonoboUserId` (exported from `bonobo-plugin-sdk/frontend`), a branded string, so cast a plain id you got from the SDK with `as BonoboUserId` when you pass it back; a plugin has no `convex` package of its own to import the id type from. A write's `_nay.message` union lists the exact refusal texts of the app version the SDK was generated from. In the app, `pnpm run generate:plugin-sdk-types` rewrites the file and the lint fails while it is stale, so a release of this package always ships the types of the app it was cut from.
