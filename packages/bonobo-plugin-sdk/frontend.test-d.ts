...
export async function http_type_check() {
	...
	// `?.` on purpose: every member's body is `| null`, and without it the "possibly null" error
	// alone would satisfy this line even if the union stopped separating the statuses.
	void page.body?.items;

	if (page.status === 200) {
		// `body` is the route's 200 shape or `null`, because an answer whose text is not JSON
		// resolves with no body. The caller has to allow for that on every status.
		const items: unknown[] = page.body?.items ?? [];
		void items;

		// @ts-expect-error and it cannot be read without allowing for the `null`.
		void page.body.items;
	}
