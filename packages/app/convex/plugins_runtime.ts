...
export async function plugins_runtime_db_enqueue_upload_completed_runs(
	...
	const serviceAttempt = await ctx.db
 ...
 * Only durable output needs the transactional archival recheck. The publish mutations
 * (`publish_file_write`, `publish_file_fill`, `publish_file_touch`) and `start_run_activity`
 * each perform it in the mutation that writes the durable output.
 */
