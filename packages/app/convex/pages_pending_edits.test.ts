...
async function seed_page_with_markdown(args: {
	...
	const snapshotId = await ctx.db.insert("pages_yjs_snapshots", {
		...
		pageId: pageId,
		sequence: 0,
		snapshotUpdate: pages_u8_to_array_buffer(encodeStateAsUpdate(baseYjsDoc)),
		createdBy: userId,
		updatedBy: String(userId),
		updatedAt: Date.now(),
	});
...
async function read_page_markdown_from_yjs(args: {
	...
	const yjsDoc = pages_yjs_doc_create_from_array_buffer_update(snapshot.snapshotUpdate, {
...
async function build_page_diff_update_from_snapshot(args: { ctx: MutationCtx; pageId: Id<"pages">; markdown: string }) {
	...
	const baseYjsDoc = pages_yjs_doc_create_from_array_buffer_update(snapshot.snapshotUpdate);
...
describe("upsert_pages_pending_edit_updates", () => {
	...
	test("upsert_pages_pending_edit_updates keeps a pending row for trailing whitespace at EOF", async () => {
		const t = test_convex();
...
describe("pages_db_reschedule_pending_edit_cleanup_for_user", () => {
...
describe("persist_pages_pending_edit_rebased_state", () => {
...
describe("remove_pages_pending_edit_if_expired", () => {
