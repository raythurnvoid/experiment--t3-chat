...
async function do_create_page(
	...
	if (args.markdown_content) {
		const editor = pages_headless_editor_create();
		pages_headless_editor_set_content_from_markdown({
...
export const try_get_markdown_content_or_fallback_to_yjs_data = query({
	...
	handler: async (ctx, args) => {
		...
		do {
			...
			if (!page || !page.markdown_content_id || !page.yjs_last_sequence_id) return null;
			...
			if (!lastYjsSequenceDoc) return null;
			if (!markdownContentDoc) return null;
			...
			if (!page.yjs_snapshot_id) return null;
			...
			if (yjsUpdatesDocs.length === 0) return null;
...
function yjs_update_from_tiptap_editor(args: {
	mut_yjsDoc: YDoc;
	tiptapEditor: Editor;
