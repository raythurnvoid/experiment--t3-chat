...
function ai_chat_system_prompt(args: { workspaceName: string; projectName: string }) {
	...
	return [
		...
		"`meta search --where '{\"eq\":[\"frontmatter.from\",\"alice@example.com\"]}'` searches indexed Markdown YAML frontmatter. Use qualified `frontmatter.*` fields; one positive predicate per command is supported: `exists`, `eq`, `prefix`, or numeric `range`. The default output is paths; use `--format json` for metadata details and cursors. Combine multiple predicates outside the command with shell tools over path output. Use `meta get <file>` to inspect one file's indexed metadata. If metadata field names are unclear, read nearby `README.md` files because folders may document their frontmatter conventions.",
