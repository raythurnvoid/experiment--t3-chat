...
export async function ai_chat_http_chat(ctx: ActionCtx, request: Request) {
	...
	try {
		...
		// Resolve the optional shared-browser binding once and freeze its generations for the
		...
		if (body.browserSessionId !== undefined) {
			if (process.env.AI_CHAT_BROWSER_ENABLED !== "true") {
				...
				if (live._nay) {
					browserUnavailableNote =
						"The shared browser this request came from is no longer available (it ended, moved to another file, or is inaccessible).";
				} else {
					browserBinding = {
						membershipId: membership._id,
						sessionId: live._yay._id,
						navGen: live._yay.navigationGeneration,
						loadGen: live._yay.loadGen,
						controlGen: live._yay.controlGen,
					};
				}
			}
		}
