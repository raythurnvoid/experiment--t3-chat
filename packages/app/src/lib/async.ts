...
export class CoalescedRunner {
	...
	async flush<T>(task: () => Promise<T>) {
		// Keep re-queueing the task until it executes or the runner is disposed.
		while (true) {
			const runResult = await this.run(task);
			if (!runResult.aborted || this.isDisposed) {
				return runResult;
			}
		}
	}
