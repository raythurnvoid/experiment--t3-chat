...
function openSseResponse() {
	...
	return {
		...
		write(chunk: Record<string, unknown>) { controller.enqueue(encoder.encode(`data: ${JSON.stringify(chunk)}\n\n`)); },
...
describe("FileEditorSidebarAgent thread upgrade", () => {
	...
	test("keeps the real composer and focus through persistence during a queued edit", async () => {
		...
		render(<div style={{ width: 480, height: 720 }}><FileEditorSidebarAgent rootTabId="app_file_editor_sidebar_tabs_agent" /></div>);
		...
		mocks.threads = [{
			_id: "thread_persisted", _creationTime: 1,
			organizationId: mocks.tenant.organizationId, workspaceId: mocks.tenant.workspaceId,
			clientGeneratedId: optimisticId, title: "Persisted", archived: false, starred: false,
			runtime: "aisdk_5", stateId: null, createdBy: "user_test", updatedBy: "user_test", updatedAt: 1,
		} as ai_chat_Thread];
		mocks.listeners.forEach((listener) => listener());
		await waitFor(() => expect(app_local_storage_get_value(
