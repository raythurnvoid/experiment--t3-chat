...
export function PageEditorRichTextBubble(props: PageEditorRichTextBubble_Props) {
	...
	const shouldShow: NonNullable<EditorBubbleProps["shouldShow"]> = (params) => {
		...
		// If focus isn't in a managed area
		// (so not in a hoisted element, the editor or the bubble itself),
		// close.
		if (!isElContainedInManagedAreas(document.activeElement)) {
			return false;
		}

		// TODO: this code was generated by GPT 5.2, but to me it did not make any sense, the bubble
		// seems to be working fine, if all good we can remove
		//
		// If TipTap passed but Novel failed, Novel rejected (image/node selection) → keep hidden.
		// if (tiptapResult) {
		// 	return false;
		// }
		//
		...
		// - Novel says "hide"
		// - TipTap says "hide"
		// - Focus is in a managed area
		//
		// The only reason TipTap could be hiding (while we still want the bubble) is the *focus* gate.
		// So we keep TipTap's non-focus checks and Novel's extra checks.
		// const { state, from, to, editor } = params;
		// const { doc, selection } = state;
		// const isEmptyTextBlock = !doc.textBetween(from, to).length && isTextSelection(selection);
		//
		// if (selection.empty || isEmptyTextBlock || !editor.isEditable) {
		// 	return false;
		// }
		//
		// // Keep Novel's additional checks.
