...
import React, { useState, useImperativeHandle, type Ref, useEffect } from "react";
...
function PageEditorPresenceSupplier(props: PageEditorPresenceSupplier_Props) {
	...
	useEffect(() => {
		if (presenceSessions && presenceData && presenceSessionsData && presence.sessionId) {
				...
				const timeoutId = setTimeout(() => {
					const presenceStore = new pages_PresenceStore({
						...
						onSetSessionData: (data) => {
							if (!presence.sessionId || !presence.roomToken) {
	...
	}, [presenceSessions, presenceData, presenceSessionsData, presence.sessionId]);

	return presenceStore ? children({ roomId, presenceStore }) : null;
}
