...
type PluginsUiFrame_Props = {
	...
	/**
	 * Builds the `context` object posted in bonobo:init. Wrap in `useFn` for the same reason as
	 * `mintSession`.
	 */
	...
	useLayoutEffect(() => {
		...
		const handle_message = (event: MessageEvent) => {
			...
			if (message.type === "bonobo:ready") {
				handle_ready();
			} else if (message.type === "bonobo:token-refresh-request" && is_refresh_request_id(message.requestId)) {
