...
export function bash_find_command_create(ctx: ActionCtx, dbFilesRoots: bash_DbFilesRoots) {
	...
	return defineCommand("find", async (args, commandCtx) => {
		...
		if (parsed._yay.prefix == null && pathResolution.kind === "external_mounts_root") {
			...
			// Path word search at the root supports the full fan-out subtree only; depth
			// filters need a single mount's folder anchor.
			if (pathQuery != null && (parsed._yay.maxDepth != null || parsed._yay.minDepth != null)) {
				return {
					stdout: "",
					stderr:
						"find: path word search at /.mounts does not support -maxdepth/-mindepth; scope to one mount: find /.mounts/<name> --path-query QUERY\n" +
		...
		if (parsed._yay.prefix != null) {
			...
			// Targets scan from their db-files path. External prefixes keep the prior raw-path
			// behavior on the workspace FS.
			...
			if (prefixResolution.kind === "external_mounts_root") {
				return {
					stdout: "",
					stderr:
						"find: --prefix cannot scan the /.mounts root; scope to one mount: find --prefix /.mounts/<name>/<path>\n" +
						"Run 'ls /.mounts' to list the available mounts.\n",
					exitCode: bash_COMMAND_EXIT_USAGE,
				};
			}
