...
export function ai_chat_tool_create_bash(
	...
	return tool({
		description: dedent`\
			...
			/tmp persists across Bash calls in this chat and reloads from Convex if the warm backend runtime cache is gone. It is not shared with new chats and is not app file storage; use app file tools for durable user-visible files. Inside a background job (&), /tmp is a private copy; its writes are dropped when the job ends and the job's stderr names the dropped paths.
