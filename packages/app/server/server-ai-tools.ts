...
export function ai_chat_tool_create_bash(
	...
	options: {
		...
		/**
		 * Agent mode only: the model a job wakeup runs with, and the hook `wait` reaches when it
		 * stopped polling for an armed job, so the chat run ends the turn. `null` leaves the
	...
	return tool({
		description: dedent`\
			...
			Background jobs: cmd & starts a background job and is the only way to start one; cmd1 && cmd2 is not a background job, and the shell never backgrounds a command by itself. & binds to the last statement only, so echo a; sleep 1 & backgrounds sleep 1 and runs echo a in the foreground; to put several commands in one job write { cmd1; cmd2; } & or cmd1 && cmd2 &. A job starts in the cwd at the &, so cd docs; ls & lists docs. The launch line goes to stderr, stdout stays empty, $? is 0, and $! is the job number in that call only (0 in the next call; use jobs to find a job later). A job gets a copy of the shell state and cannot change the shell (cd or x=1 inside a job does not reach the shell); extra file descriptors are closed with a warning. A job runs a script and ends: no servers, no ports, no curl localhost. A job's /tmp is a private copy that is dropped at a pause and at the end; see the /tmp rules above for where a job's files and output go. A job runs one top-level statement at a time, and each run of it has an 8-minute budget: when the budget is nearly used, the job pauses after the current statement and continues in a new run with its variables, functions and cwd, so a script of several statements is not bound by the budget, but one statement is (a single loop or command must finish within 8 minutes). The statements of a job written as { cmd1; cmd2; } & count as top-level. A top-level sleep N with a literal N of 5 seconds or more pauses the job for N seconds (at most one hour) without holding a worker; a shorter sleep, sleep $var, or a sleep inside a loop, a pipeline or a function runs normally. A paused job shows as queued in jobs and in Notifications, the transcript gets one entry per run of the job, and jobs -o N shows the head of the whole job's output. At most 4 of your own jobs in this workspace can be queued, running or stopping at once; a refused launch prints a line starting with "bash: cannot start a job: " on stderr, $? is 1, and set -e stops the call. Only the 4-jobs refusal is worth waiting out; a script over 64 KiB, a shell state over 128 KiB, and a launch from a job that is already stopping are refused every time. Two jobs copying at once share one copy lane, so put sequential cp or mv commands in one job: the second job's copy waits up to 60 s for the lane and then fails with exit 1. A job may start jobs; they are independent, and stopping a job does not stop the jobs it started. jobs lists the live jobs of this chat and jobs -a the newest 8; both print status words and both exit 3 while any listed job is live. jobs -o N prints the stored stdout then stderr of job N, then one final [job N exit C] line on stderr that is the job's exit code, not part of its own output (32 KiB each with a [truncated] line, at most two reads per call; the read itself exits 0, or 1 when the job is unknown or has no stored output, then read the transcript). While job N runs, jobs -o N prints the output it has produced so far (updated every 5 seconds, the same 32 KiB per stream) with a final [job N running] line instead, and exits 3; the transcript gets the full output when the job ends. wait waits up to 30 s for the jobs this call started (0 at once when it started none); wait N waits for the named jobs, which may be any of your jobs in this chat; wait -t SECONDS changes the bound. wait is a shell builtin, so which wait finds nothing even though wait works. wait normally returns 3 (still running) because a job may take minutes; run jobs in a later call instead of waiting again. Otherwise wait returns the worst exit code of the waited jobs. In Agent mode, set wakeOnJobFinish: true on a Bash call when the jobs it starts or waits for should wake you when they end: a finished job then adds a system message with its number, shell, exit code and the head of its output, and starts a new run of yours (only when no run is active; during a run the next Bash call prints the job note as usual). In such a call wait does not poll: it prints bash: waiting for job N on stderr, exits 3, and you must end the turn with a short status; the job's finish starts your next run. kill N asks a job to stop; a stopped job reports 143 (status stopped), a job that used its whole budget reports 124 (timed out). When a job finishes, the next Bash call prints bash: job N done|failed|timed out|stopped. Output: /shells/<name>/transcript on stderr, so the note also tells you the outcome. Stopping the chat leaves jobs running; the Notifications panel lists every job with its Stop button. fg, bg, disown and suspend are unavailable (127).
		...
		inputSchema: z.object({
			...
			shell: z
				...
				.optional()
				.describe("Named shell to run in. Omit for default. A new name creates the shell. At most 10 per chat."),
			...(options.jobWakeup
				? {
						wakeOnJobFinish: z
							.boolean()
							.optional()
							.describe(
								"Wake the agent when the background jobs this call starts or waits for end. A finished job then adds a system message with its result and starts a new run, if no run is active. With this set, wait stops polling and the turn ends.",
							),
					}
				: {}),
		}),
...
export function ai_chat_tool_create_edit_file(
	...
	return tool({
		description: dedent`\
			...
			Usage:
			- The path must refer to an existing editable text file (absolute, starting with "/"). A file is editable text when its stored content type is text (Markdown, plain text, JSON, YAML, CSS, JavaScript, and similar), whatever its name. Images, PDFs, and other stored files are refused.
	...
	return tool({
		...
		inputSchema: z.object({
			...
			set: z
				.array(
					z.object({
						key: z.string().describe('The metadata key, for example "created-by".'),
						value: z.union([z.string(), z.number(), z.boolean()]).describe("Text, number, or true/false."),
					}),
				)
				.describe("Keys to set. An existing key keeps its place in the map; a new key is appended."),
			remove: z
				.array(z.string())
				.describe(
					'Keys to delete. Pass the bare key, for example "status", never the search field name "metadata.status". Pass an empty array when there is nothing to delete.',
				),
		}),
		...
		execute: async (args) => {
			...
			if (written._nay) {
				if (written._nay.name === "read_only") {
					throw new Error(
						`Cannot set metadata on ${normalizedPath}: ${written._nay.message} Do not retry this path with another write tool.`,
						{ cause: written._nay },
					);
				}
