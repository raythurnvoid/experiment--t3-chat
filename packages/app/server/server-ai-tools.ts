...
export function ai_chat_tool_create_bash(
	...
	return tool({
		description: dedent`\
			...
			File writes, redirects, and deletes under ${currentWorkspacePath} are not supported shell operations; rm and ln are not available for app files. mv <app-path> <app-path> proposes a pending move/rename (one source only, never overwriting an existing destination): the committed path changes only after the user accepts it in the Files sidebar, and until then reads keep showing the old path. cp <app-file> <new-app-path> proposes a pending copy (one source only, never overwriting an existing destination): the destination file appears immediately with the copied content pending review, your reads at the destination show that pending content, accepting publishes it, and discarding removes the destination file. cp <app-file> /tmp/<name> stays an immediate durable per-thread scratch copy. Persistent Markdown edits belong in write_file or edit_file with app paths such as /docs/readme.md. If a user asks to delete a file, explain that deletes are not available as shell operations.
