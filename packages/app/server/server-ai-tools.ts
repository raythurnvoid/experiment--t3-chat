...
export function ai_chat_tool_create_bash(
	...
	return tool({
		description: dedent`\
			...
			Use meta search --where '{"eq":["frontmatter.from","alice@example.com"]}' to search indexed Markdown YAML frontmatter. Fields must be qualified frontmatter.* names; one positive predicate per command is supported: exists, eq, prefix, or numeric range. Default output is paths; use --format json for metadata details and cursors. Combine multiple predicates outside meta with shell tools over path output. Use meta get <file> to inspect one file's indexed metadata. If field names are unclear, read nearby README.md files because folders may document frontmatter conventions.
