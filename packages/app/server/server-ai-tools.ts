...
export function ai_chat_tool_create_bash(
	...
	return tool({
		description: dedent`\
			...
			In Agent mode, shell writes under ${currentWorkspacePath} create pending proposals the user reviews in Files, exactly like edit_file: create or overwrite a file with a quoted heredoc (cat > '<path>' <<'EOF' ... EOF) or a redirect, append with >>, tee writes each app target as a proposal, and touch on a new path creates an empty-file proposal (touch on an existing app file changes nothing). Your own reads (bash and the file tools) see your pending proposals as if applied, while other users and the Files UI see the committed tree until the user accepts (a brand-new file appears to everyone right away as an empty placeholder). In Ask mode app files are read-only. Deletes are not supported: rm and ln are not available for app files. mv <app-path> <app-path> proposes a pending move/rename (one source only); accepting a move onto an occupied path replaces that file. Plain mv never overwrites an existing destination; mv -f <app-file> <existing-app-file> proposes a replace: the destination file keeps its identity and gets the source's content as a pending content replacement, and accepting saves that as a new version of the destination and archives the source file (mv -f replaces files only; a plain folder move can replace an empty folder, and folders never replace files or the reverse). cp <app-file> <app-path> proposes a pending copy (one source only): a new destination file appears immediately with the copied content pending review, your reads at the destination show that pending content, accepting publishes it, and discarding removes the destination file. When the cp destination file already exists, the copy becomes a pending content replacement on that file, and discarding keeps the destination file with its committed content. cp <app-file> /tmp/<name> stays an immediate durable per-thread scratch copy. Targeted edits to existing Markdown belong in edit_file with app paths such as /docs/readme.md. If a user asks to delete a file, explain that deletes are not available as shell operations.
			Convert bash paths under ${currentWorkspacePath} to app paths for edit_file by removing the current workspace path prefix. Preserve the full remaining suffix: ${currentWorkspacePath}/folder/README.md becomes /folder/README.md, never /README.md.`,
		inputSchema: z.object({
			command: z
				.string()
				.min(1)
...
// write_file is deactivated for new generations via BASH_REPLACED_TOOL_NAMES in convex/ai_chat.ts
// but stays here so Ask mode strips edit_file and historical write_file messages keep validating.
...
export function ai_chat_tool_create_write_file(
	...
	return tool({
		...
		execute: async (args) => {
			...
			const newText = files_normalize_ai_edit_content(files_normalize_lf_newlines(args.content), oldText);
