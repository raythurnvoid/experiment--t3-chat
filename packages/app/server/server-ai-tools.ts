...
export function ai_chat_tool_create_bash(
	...
	return tool({
		description: dedent`\
			...
			Bash starts in the current project path at ~/w/${ctxData.workspaceName}/${ctxData.projectName} (${currentProjectPath}). ~ is ${HOME}, the app mount is ${appMountPath}, and /tmp is in-memory scratch that resets between bash calls.
			...
			Convert bash paths under ${currentProjectPath} to app paths for write_file/edit_file by removing the current project path prefix.`,
		inputSchema: z.object({
			command: z
				.string()
				.min(1)
