...
export function ai_chat_tool_create_bash(
	...
	return tool({
		description: dedent`\
			...
			Use find <path> --limit N [--cursor CURSOR] for subtree pages, find --prefix <prefix> --limit N [--cursor CURSOR] for raw startsWith path discovery, and find --extension EXT or find -name PATTERN for extension/name discovery. find -maxdepth N and find -mindepth N filter results by depth. In -name/-iname the PATTERN accepts glob wildcards (quote it, e.g. -name '*.md'), even though bare glob operands like ls *.md are not. With filters, --limit bounds entries SCANNED per page in path order, not matches — keep running the printed Next page command even when a page shows no matches. GNU find extensions such as -printf, -mtime, -newer, -exec, and -ok are not supported; omit them.
			Use search --limit N <query...> for indexed content search only. search matches your whole query as one CONTIGUOUS, case-insensitive substring of content (not independent keywords), always across the ENTIRE workspace — no file paths or directory scoping. If a multi-word query returns nothing, retry with one short distinctive token; raising --limit past 100 has no effect. Do not pass paths to search as positional operands, and do not use search as a pipeline filter.
			Use exact app paths with cat/head/tail/wc/stat; these readers fetch at most 10 app files per command — to READ specific known files, cat them in batches of 10 or fewer across multiple commands; to FIND which files mention something, use search (it returns matching snippets, not whole files). The 10-file cap is a per-command batch limit, not a total ceiling. Large files are not read inline: cat refuses a full dump past a size limit. Read a large file in bounded pages with head -n N (first lines; prints the next sed -n page command to continue), sed -n 'A,Bp' (any line range), or tail -n N (last lines) — up to 200 lines per read; wc reports its line/word/byte counts so you know its size first (line/word counts are lower bounds for files beyond the scan window). Use search to find content across files. Pipelines with sed/awk/sort/uniq/cut work on cat output. Do NOT use | grep or | head as pipeline filters on app-file content — they do not scan app files through a pipe; use search for content filtering and head/tail with a direct path operand instead.
