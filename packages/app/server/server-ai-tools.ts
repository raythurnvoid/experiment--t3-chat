...
export function ai_chat_tool_create_bash(
	...
	return tool({
		description: dedent`\
			...
			ls --limit and find --limit are app-file pagination commands. Relative paths resolve against the current working directory. From ~, omit the path or use the mounted app path (${appFilesMountPath}/...) instead of ".".
			...
			File writes, redirects, moves, and deletes under ${appFilesMountPath} are not supported shell operations. rm, mv, and ln are not available for app files, and cp cannot write into the app tree; cp <app-file> /tmp/<name> is allowed as a one-shot scratch copy. Persistent Markdown edits belong in write_file or edit_file with app paths such as /docs/readme.md. If a user asks to delete or move a file, explain that these are not available as shell operations.
		...
		inputSchema: z.object({
			command: z
				...
				.max(20_000)
				.describe(
					`Shell command to run. Omit PATH to inspect the current app directory; use ${appFilesMountPath} only when cwd is outside the app tree or when targeting that absolute path intentionally.`,
				),
		}),
		execute: async (args) => {
