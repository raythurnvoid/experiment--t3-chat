...
import { files_tiptap_markdown_to_plain_text } from "../shared/files-tiptap.ts";
...
function set_chunk_bitmask_flags(args: {
	...
	if (codeHasMoreAbove || tableHasMoreAbove) {
		chunkFlags |= files_chunk_BITMASK_FLAGS.hasMoreFragmentContentAbove;
	}
	if (codeHasMoreBelow || tableHasMoreBelow) {
		chunkFlags |= files_chunk_BITMASK_FLAGS.hasMoreFragmentContentBelow;
	}

	return chunkFlags;
}

export async function files_chunk_markdown(markdown: string, options?: { maxChunkSize?: number }) {
...
export function files_chunk_has_bitmask_flag(chunkFlags: number, flag: number) {
