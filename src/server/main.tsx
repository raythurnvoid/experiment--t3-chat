...
app.post("/api/chat", async (c) => {
	try {
		...
		console.log("Last message:", Array.isArray(messages) && messages.length > 0 ? messages[messages.length - 1] : null);
			...
			tools: {
				...
				createArtifact: tool({
					...
					execute: async ({ type, title, language, code, markdown }) => {
						...
						summary += ". The artifact is now available in the side panel for viewing and editing.";
