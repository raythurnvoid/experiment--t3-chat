...
7. Chat file tools: only `bash`, `edit_file`, `web_search`, `execute_code` are active; `read_file`/`list_files`/`glob_files`/`grep_files`/`write_file` are registered but filtered out of generation (ai_chat.ts `BASH_REPLACED_TOOL_NAMES`) — new generations write app files with bash redirects/heredocs, while old threads still render `write_file` parts. Do not write eval criteria that require the filtered tools.
