...
21. Restricted-scope restore fixtures: build the folder so it carries its OWN restriction (`restrictedScopeNodeId === its own _id`, which `unarchive_nodes` deliberately skips) and the child so it INHERITS it (`=== the folder's _id`) — only the inheriting child exercises `authorize_leaving_restricted_scope`. Make the restore behave as a MOVE by archiving the parent and restoring the CHILD alone: the parent is still archived, so it relocates to root. Neither pointer is visible in the sidebar; read `path`/`parentId`/`restrictedScopeNodeId` from `list_tree`. The org OWNER bypasses every permission check (the Share dialog says so: it lists the owner as `Full access`), so an owner-only run proves NO OVER-REFUSAL and never proves the refusal works — that side needs a second member with a `content.write` grant, and one browser holds one signed-in user, so leave it to `convex/access_control.test.ts`.
