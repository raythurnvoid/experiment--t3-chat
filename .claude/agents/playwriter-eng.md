...
9a. Eager-created pending files show a green `Added` badge in the tree: `.FilesSidebarTreeItemSecondaryContent-added`, cleared on accept (row stays, committed) or discard (row hard-deletes). Scope badge probes per row — `treeitem.closest(".FilesSidebarTreeItem").querySelector(".FilesSidebarTreeItemSecondaryContent-added")` — a loose ancestor walk (`closest("[class*=FilesSidebarTreeItem]")?.parentElement`) reads a SIBLING row's badge and false-fails after-accept checks.
