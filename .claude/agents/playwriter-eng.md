...
6. Files tree rows are `[role="treeitem"]` with `aria-label` = node name (no `data-item-id`; anchor by label); committed create/rename flow: New file/New folder button creates a committed `new-file.md` immediately (no dialog, no auto-rename), then click the row, `F2` → focused `.FilesSidebarTreeItemTitle-input` → `Control+a` → type → Enter. Row title inputs exist for every row; only the focused one is editable. Per-row menu: button `More actions for <name>` → menuitems Copy path / Rename / Archive.
