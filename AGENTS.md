...
- Do not read a verification command's exit code through a pipe. `cmd 2>&1 | tail -20` reports `tail`'s status, not the command's, so a failing gate is announced as exit 0 while the error text scrolls past above the lines you kept. Observed on `build:verify`, which ended in `ELIFECYCLE Command failed with exit code 1` and was reported as success. Either run the command bare, or redirect to a file and read the status on the next line: `cmd > "$TEMP/out.txt" 2>&1; echo "EXIT: $?"`. Then read the command's own summary line too, because a tool can report failure in its output while still exiting 0.
...
- Dev servers: check whether one is already running before you start anything, and never start a second copy on another port. The user runs these themselves sometimes, so a duplicate on port 5174 or a second Convex watcher is the real mistake here, not starting one at all.
  - Check the ports and processes: `netstat -ano | grep -E ":5173|:3210"` for Vite, and `Get-CimInstance Win32_Process -Filter "Name='node.exe'" | Where-Object { $_.CommandLine -match 'convex|vite' }` for what is actually running. Read the command line, not just the port — `convex mcp start` matches `convex` and is NOT the watcher.
  - The user's own Cursor terminals are readable at `C:\Users\rt0\.cursor\projects\c-Users-rt0-Documents-workspace-rt0-t3-chat\terminals\*.txt`. Use it to see which commands the user started; it shows their integrated terminals only, so a server started anywhere else does not appear there. Port and process inspection is the authority.
...
- `packages/native-popovers` is a first-party submodule (repo `raythurnvoid/react-native-popovers`) and a pnpm workspace package. `MyTooltip` is built on its `native-popovers/tooltip` entry, and the React Compiler also compiles its `src/`. Its CSS layer `native_popovers` must stay after `base` in the `app.css` layer order. Change it in its own repo: commit and push there first, then commit the new gitlink here.
...
Dev data costs money, and the dev deployment already holds thousands of test files. Before any task creates data there (QA, evals, fixtures, uploads, second users, new orgs or workspaces), load `.agents/skills/qa-data/SKILL.md` and test with the existing files in its catalog. Create new data only when nothing fits, and add it to the catalog.
...
A check that needs a second identity — a permission refusal, a share grant, presence — cannot be done from the signed-in account, because the organization owner passes every permission check and an owner-only run proves nothing about a refusal. Never sign in or out in the user's own browser profile: Clerk runs in single-session mode here, so that would sign the user out of their own session. Run second identities in an isolated scratch browser instead. Load `.agents/skills/app-playwriter-harness/references/second-user-fixtures.md`. Prefer a seeded account: every new anonymous visitor adds a user, a personal org, and a workspace that stay in the database. Use a fresh anonymous user only when the check is about anonymous users; an invite by `userIdToAdd` turns it into a normal non-owner member. For the seeded accounts, load `.agents/skills/app-playwriter-harness/references/clerk-test-accounts.md` — the dev database holds seeded `+clerk_test` QA accounts whose fixed public test code (`424242`) is fixture data, not a credential, and logging in and out with them in the scratch browser is a supported autonomous flow.

Before trusting any browser result about Convex code, check that the browser runs your working tree. `pnpm run dev` starts Vite only; the app then talks to the deployment already in `packages/app/.env.local`, so edits under `convex/` reach the browser only while `convex dev` is pushing them. If no watcher is running, start one — see the dev-server rules above. A missing watcher is never a reason to skip the check or to fall back to CLI-only evidence. Prove it once per session by breaking one check on purpose and watching the app's behaviour change. Without that, a green browser run says nothing about your edits.
