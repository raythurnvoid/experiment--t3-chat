...
- [2026-02-13] Chat edit on `/pages` Agent panel: editing a right-side user message via `.AiChatMessageUser-edit-button` can send `POST /api/chat` with `parentId` as `ai_message-*`, which deterministically returns 500 `Failed to reconstruct messages` when that ID is not resolvable server-side. Evidence: Playwriter capture showed request `threadId: "n17006qz14g30ykb3kjb6bm3ts811svm"` and response payload containing the same `ai_message-*` in `messageId`.
- [2026-02-13] Chat edit parent linkage: editing and resubmitting from `.AiChatMessageUser-edit-button` now posts `parentId` as Convex-style IDs (`n...`) and returned `200` in repeated runs, including stop/resend flow, with edited text remaining visible. Evidence: 4 captured `POST /api/chat` requests all used `parentId` `n57dwab0faht1jaecr2nd5w9d5813q9z`; captured responses were `200` with no observed `500`.
- [2026-02-13] Root-branch chat edit semantics: when editing the first/oldest user message (no assistant anchor), `POST /api/chat` can carry `parentId: null` and still stream successfully with `200`; classify this as root-branch behavior, not missing-parent failure. Evidence: repeated first-message edits captured `parentId` null with SSE `start` events and no `Failed to reconstruct messages`.
- [2026-02-13] Stop verification fallback: if no stop control renders, verify stream start by checking `/api/chat` SSE payload for `data: {"type":"start"}` and report stop as unavailable separately from transport failures. Evidence: root-edit requests returned streaming `start` events while both `.AiChatThread-composer button:has(.AiChatComposer-cancel-icon)` and `.AiChatComposer-cancel-icon` remained absent.
- [2026-02-13] Composer interaction speed path: for the bottom chat composer on `/pages`, `.AiChatThread-composer .AiChatComposer-editor-content` is reliably writable with `locator.fill(...)` when click targeting is flaky due overlapping controls; this avoids pointer-interception retries and still submits via the normal send button. Evidence: click-based input timed out with intercepted pointer events, while `fill` + `button[aria-label="Send message"]` produced a successful `POST /api/chat`.
- [2026-02-13] Root composer stop targeting: the most reliable stop probe is scoping to `.AiChatThread-composer .AiChatComposer-cancel-icon` and clicking `closest("button")` after `scrollIntoViewIfNeeded()` on `.AiChatThread-composer`; if icon count stays `0` through the poll window, treat Stop as unavailable rather than selector drift. Evidence: repeated root-edit stop polls with root-composer scoping found icon count `0` while `/api/chat` still returned successful streaming `200` responses.