...
Workspace AGENTS.md and skill loading use saved sources in `convex/ai_chat_context.ts`, with no pending content or move overlay. Any author's eager-created source stays excluded while its saved sequence equals the creation stamp. Changes apply on the next turn after saving; agent proposals must be accepted and saved. Ordinary task-file reads keep the flow below. See the [workspace skills spec](../ai-chat-skills/SKILL.md).
