...
- `files_pending_updates`
  ...
  - optional canonical content group, always set together:
    ...
    - optional `contentNeedsRebase: true` after a collaboration toggle. The old base and all three states stay intact until Review prepares them. Removing the content group also removes this flag.
...
**Files with collaboration turned off go through the same steps with an asset base.** The file has no Yjs document, so the pending doc stores no `baseYjsSequence` or `baseLineageGeneration`. It stores `baseAssetId` instead: the content asset the branches were built from (`files_pending_update_has_asset_content`; `files_pending_update_has_content` accepts both shapes). The upsert reads the committed text and builds the base and staged branches from it, in the file's own `yjsRootKind`; the agent's text becomes the unstaged branch. The proposal is stale when `baseAssetId !== node.assetId` (`files_pending_update_content_is_stale`): a member saved the file after the agent made the proposal. Then the read overlays (cat, wc, grep, the by-path state door, text and metadata search) serve the committed text and hide the proposal; the public upsert refuses with `files_PENDING_UPDATE_STALE_BASE_MESSAGE` while the agent's internal upsert rebuilds the branches from the saved text; Accept refuses with the same sentence; Discard deletes the doc (or degrades it to a move-only or delete-only doc). Accept (`save_file_pending_update`, which dispatches to `action_save_file_pending_update_non_collaborative` and its mutation `save_file_pending_update_non_collaborative_in_db`) writes the staged text as the committed text, stores a version snapshot, and bills one `file_save`; a partial accept keeps the doc with the remaining unstaged hunks and moves `baseAssetId` to the new asset. Collaboration toggles preserve proposals and require preparation during Review, as described below. An accepted whole-file copy and a replacement-style version restore (`db_install_file_content_replacement`) still drop them for every member. The tool output is the same as for a collaborative file: the write is a pending change the user reviews in Files. Normalization is the same too (one leading BOM removed, line endings to LF), and a Markdown proposal is rendered through the rich text document, so Accept on a `.md` file applies Markdown's trailing newline and its other serialization changes. `mv -f` does not look at collaboration mode at all: it is a structural replace-move, so accepting archives the target and moves the source, with its own mode and history, into its place. See the `files-editable-text` skill for the mode itself.

In the UI, a proposal on a file with collaboration off opens in `FileEditorDiff` with `nonCollaborative` and `committedAssetId` (the `assetId` of the editor node `FileNodeView` chose for the route: the selected file, or a folder's README): the editor loads only the proposal's branches, so there is no live-file fetch, no Sync, and no versions button. Save calls the same `save_file_pending_update` and shows "Changes saved". The action result carries `pendingUpdateUpdatedAt` (the doc's `updatedAt` after the save, or null when the save deleted the doc), and the view stays busy until its doc query shows that doc, because the query can deliver the save later than the action result and the reconcile effect would otherwise reload state pages the save deleted. The view exits once the doc is gone (a full save, a Discard, a row action, another tab). These exits replace the current browser history entry, so Back does not reopen the finished review. A failed branch reload, including a thrown query error, shows "Failed to load the updated proposal. Open it again." and exits the same way. An ordinary stale asset proposal renders read-only, with a `role="status"` line carrying the stale sentence and a single "Discard proposal" button. The pending row shows the caption "Out of date", appends ", out of date" to the row link's name, keeps Accept enabled with the sentence as `title` (a click shows it as a toast and sends nothing), and "Accept all" skips stale rows. Marked proposals use the preparation flow below.
Saving in the normal editor has no warning about making a proposal stale. The out-of-date notice after Save gives that feedback.

**Proposals kept across collaboration changes.** Both toggles mark every content proposal with `contentNeedsRebase: true`. The mark keeps the old base pointers, all branch states and chunks, contributor threads, and move/delete intent. It leaves `updatedAt` and the current expiry task unchanged. Saved-sequence markers are deleted even when there is no pending content. Structural-only and whole-file copy proposals are not marked.

Marked content still counts in Pending and remains readable in Review. Normal agent reads and text/frontmatter search use committed content. Upsert, Sync, Save, and their final commits refuse marked content. The agent's ordinary stale-base rebuild also refuses; it cannot overwrite a kept proposal. Pending deletes can still be accepted. Discard proposal removes the text changes and keeps any pending move or delete.

Opening Review calls `prepare_file_pending_update_after_collaboration_change` for the current user's exact proposal. It merges the old base-to-staged and base-to-unstaged text changes into the current file separately. Different overlapping edits or different inserts at the same point refuse preparation. Identical changes are kept once. A diff-budget, size, shape, or frontmatter failure keeps the old proposal untouched. Old raw Yjs updates are never merged into the new document history.

Successful preparation builds new branches from the current target, seals their pages, and commits only if the source proposal and target still match. The guards include the source timestamp and three state ids, current access/lock/shape, and the exact live sequence-doc id/counter/generation or current asset. The commit retires old state ownership before activating the replacement in one transaction. It clears the flag and the unused base kind, updates indexes, and refreshes normal expiry. A no-change result settles under the same target checks. Preparation does not save file text or bill a file save.

The review UI blocks typing, hunk actions, Save, Sync, and delayed draft writes while preparation or a history change loads new branches. It handles mode changes in the same mounted editor. Old pane writes carry the `reviewedUpdatedAt` of the version they loaded. A preparation failure offers Retry and Discard; conflicts also offer Copy accepted text and Copy proposed text. Resolve a conflict by copying both proposal texts, saving the chosen result in the normal editor, then discarding the old proposal. Unsent local typing is kept for copying when panes must be replaced. Bulk Accept skips marked content and explains why.

`get_file_pending_update` returns `currentYjsLastSequenceId` beside the proposal fields. It reads the node and proposal together; the extra field is query metadata, not stored proposal data. Loaded branches use this id instead of a separately delivered node prop. Review waits until both agree. Live fetches also check their returned document id before Sync merges bytes or a late Save/Sync refresh replaces the file text in the UI.
...
- Upsert and rebase: `upsert_file_pending_update` (action), `upsert_file_pending_update_internal_action`, `commit_file_pending_update_upsert_in_db`, `persist_file_pending_update_rebased_state` (action), `prepare_file_pending_update_after_collaboration_change` (action), `commit_file_pending_update_rebase_in_db`, `settle_file_pending_update_no_change_in_db`, `refresh_file_pending_update_in_db`
...
- Every proposal edit refreshes the 4-hour expiry, including the identical-content short-circuits (upsert and sync). An identical re-write still bumps `updatedAt`, reschedules cleanup, and records new structural intent (`copiedFrom` changes and a missing `eagerCreated` stamp still land). Toggle marking is the exception: it leaves the timestamp and expiry task unchanged. Successful preparation refreshes them normally.
...
- Every edit that leaves a pending update doc alive refreshes its four-hour cleanup task. This includes content upserts, move upserts, rebases, preparation, partial saves, and structural accept/discard paths that preserve part of a content-plus-move doc. Toggle marking alone leaves the existing deadline unchanged.
...
- Before Save or Sync, the `base`, `staged`, `unstaged` and live documents must share the Yjs history used by state-vector diffs. Marked proposals deliberately keep an older history and must pass text-based preparation first. Start rebuilt branches from the exact current base. Never diff an independent fresh `Y.Doc` against the live document: state vectors compare Yjs structs and client clocks, not visible text, so that can duplicate the document.
- Normal rebase and reconciliation use text and raw state-vector diff updates within the same document history. Preparation after a toggle merges text into the new history instead. Both use the shape-aware dispatchers (`files_yjs_doc_get_text` / `files_yjs_doc_update_from_text`). The only ProseMirror-touching code on these paths is the `rich_text` branch inside the dispatchers themselves.
...
- Toggle both ways with plain-text and Markdown proposals. Keep exact proposal ids, state ids, source fields, and expiry until Review prepares them.
- Verify partial acceptance, a separate live edit, and another save. Assert exact text without lost or duplicated edits.
- Verify conflicts, late action results, locks, multiple owners, and mode changes while review stays open. Failed preparation keeps the old proposal readable.
- Verify marked content is hidden from agent reads and searches, and agent writes refuse without changing it. Bulk Accept skips it; pending deletes still work.
