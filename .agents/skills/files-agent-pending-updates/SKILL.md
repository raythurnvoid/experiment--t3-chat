...
**Files with collaboration turned off skip all of this.** They have no Yjs document, so there is no branch to build and nothing to review. `files_agent_write_file_text` takes a `nonCollaborative` boolean, and when the read step above reports `true`, it calls `internal.files_nodes_content.replace_file_content_internal_action` instead: the text the agent wrote is saved straight away. Last write wins even if another save landed after that read, and earlier saves stay in version history. The tools say so in their output, so the model does not tell the user to review a proposal that does not exist — `edit_file` ends with "Collaboration is off for this file, so the change is already saved and there is nothing to review", `cp` prints `copied: <source> -> <dest> — collaboration is off for the destination, so the copy is already saved`, and a redirect/`tee` write adds `bash: saved '<path>' immediately because collaboration is off; there is no pending change to review.` The save door runs the same ACL `content.write` check and the same read-only lock check the proposal path runs, so nothing is looser. Two consequences worth knowing: a direct save keeps the text the shell wrote after the same normalization (one leading BOM removed, line endings to LF) and parses nothing, even into a `.md` file, while the collaborative path renders the pending Markdown document and so appends Markdown's trailing newline and applies every other Markdown change; and `mv -f` does not look at collaboration mode at all: it is a structural replace-move, so accepting archives the target and moves the source, with its own mode and history, into its place. See the `files-editable-text` skill for the mode itself.
