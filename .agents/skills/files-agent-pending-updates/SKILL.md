...
- Discard is idempotent: a missing doc or a doc with nothing structural returns `_yay`, so bulk flows and already-settled file-swap cycle members just no-op.
- Deliberate non-guarantees: other members of a file-swap CYCLE apply their current destinations, and equal-base concurrent content edits stay last-write-wins.
