...
- Closing the generation hides the approved subtree at once. Each node gets a durable `files_pending_node_cleanup_tasks` doc. Cleanup removes the proposal indexes, retires its states, hands assets to the deletion ledger, and expires its batches. It keeps the node until its batches, states, and discarded children are gone. Node slots release only after node deletion.
- Each cleanup task owns exactly one scheduled job. The task stores `scheduledFunctionId` and `nextAttemptAt`, and the job takes only `privateNodeId`. A job exits when the task is gone or when `nextAttemptAt` is still in the future, because a newer job then owns the task. A job that reschedules itself never cancels its own id: Convex would then also cancel the job it schedules next. Code that moves a task from outside its job cancels the old job first.
- A node waiting on a batch, a state, or a preparing transfer item polls every 60 seconds. A node waiting only on discarded children schedules a 15-minute safety-net run. When the last discarded child is deleted, the child's job wakes the parent task at once, so a deep folder chain clears in one job per level instead of one delay per level. A node with 16 or more hidden active children closes the next 16 in a new job at once.
- A 15-minute cron restarts tasks whose `nextAttemptAt` is more than 15 minutes in the past. It cancels the old job before it schedules a new one, so a busy scheduler can never collect duplicate jobs.
...
3. Agent-mode Bash `rm` stores `pendingArchive` for saved nodes. Accept archives the reviewed node and descendants through the archive job (see the [read-only spec](../files-read-only/SKILL.md)). A big folder goes on in the background with an Activity. Each step removes the proposals on the nodes it archives, and the `pendingArchive` proposal is removed when the job ends. A Discard during the job stops it. The part already archived stays archived. A folder with more than 2,000 items is too big for the proposal-time read-only check, so `rm -r` still stores the proposal and the accept job checks every item. Removing an owned private create discards it without creating a saved node. Normal flags still apply: folders need `-r`, and `-f` silences missing paths.
