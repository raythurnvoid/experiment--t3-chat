...
- The same rule applies to per-row UI subscriptions: many small per-id `useQuery`/`useQueries` subscriptions (for example one write-permission query per visible file row) are the preferred shape, not a problem to batch away. Each entry caches and invalidates on its own, and every other surface asking about the same id shares the same cache entry.
- Memoize the descriptor object passed directly to `useQueries`, including an empty `{}`. Its React subscription depends on that object's identity. A fresh object on every render can cause a render loop. Use stable inputs for the memo and cover subscription changes with the real Convex hooks; mocking `useQueries` hides this contract.
