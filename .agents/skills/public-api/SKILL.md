...
`list` also takes an optional `keyPrefix` (printable ASCII `0x21`-`0x7E`, same validator the user-write door uses) that narrows the page to keys under that prefix as an index range; an invalid prefix answers 400. `read` and `list` responses carry `createdBy`, `updatedBy`, `ownership`, `createdAt`, and `revision` alongside the value. Member-initiated writes do not have an HTTP route at all: the host app calls the `plugins_data.user_*` public mutations directly with the member's own Convex auth — see `../plugin-system/SKILL.md#plugin-document-store`. For live reads, pages use the reactive watch/window bridge instead of polling these routes; `list` remains the non-reactive deep-history path for a member session or page that needs documents beyond the window bridge's 600-doc ceiling.
