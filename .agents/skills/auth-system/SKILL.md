...
- [data_deletion.ts](../../../packages/app/server/data_deletion.ts)
...
- `"data"` is a live account reset for local/admin cleanup. It keeps the user row live, preserves Clerk and anonymous auth state, preserves profile and billing/customer state, cancels queued deletion requests owned by that user, ensures a usable `personal` / `home` default tenant, purges content inside the default project, deletes extra personal projects, and deletes non-default workspaces/projects only when the reset user is the only active participant in that tenant scope.
- `"data_and_auth"` deletes tenant/user data and auth state, attempts Clerk deletion, removes anonymous auth tokens, keeps the final tombstoned user row, preserves `billing_usage_snapshots`, and enqueues the same retryable period-end cancellation used by the normal delete flow.
- `"data_auth_and_user_record"` deletes tenant/user data and auth state, revokes the Polar subscription immediately, deletes the Polar customer immediately, deletes the local `billing_usage_snapshots` row, and then purges the final local tombstone. Local Polar customer mapping and local subscription rows are cleared through Polar deletion webhooks (`customer.updated`/`customer.state_changed` with `deleted_at`, or `customer.deleted`).
