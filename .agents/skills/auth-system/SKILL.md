...
- Returning users do not wait for that check. When the first Clerk token already has `external_id` and no anonymous token is stored, `AppAuthProvider` marks auth ready with that id first, so Convex signs in and the app starts while `resolve-user` runs. In the normal case the route only reads and returns the same id. If it returns another id (a data reset) or `restoredDeletedAccount: true`, the provider shows the loading state, waits for a token with the right `external_id`, and remounts everything under auth (Convex provider and router) with a new key. That clears query errors from the moment the app ran as the old user. It does not undo a navigation: `app_router()` is a module singleton, so the URL stays. No startup query navigates in that window today, because `organizations.list` returns an empty list for a missing or deleted user. A `resolve-user` failure after the early start still signs the user out, but now they may see the app for a moment first. A first sign-in (no `external_id`) and an anonymous upgrade (stored anonymous token) still wait, because `resolve-user` creates or links the user there.
