...
- `users.hard_delete_user_now` is the direct admin path for immediate local hard deletion. Its `purgeUserMod` defaults to `"data"`:
- `"data"` deletes tenant/user data only, keeps the final tombstoned user row, keeps Clerk and anonymous auth state, and enqueues the same retryable period-end cancellation used by the normal delete flow.
