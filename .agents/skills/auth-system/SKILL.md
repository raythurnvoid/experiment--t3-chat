...
- `"data_and_auth"` tombstones the local user before provider cleanup, drains user-owned plugin UI sessions, schedules period-end Polar cancellation, deletes Clerk auth, finalizes local data/auth, keeps the tombstone and `billing_usage_snapshots`, then enqueues the tenant-deletion Workpool once when resource requests remain.
- `"data_auth_and_user_record"` tombstones the local user before provider cleanup, drains user-owned sessions, revokes the Polar subscription, deletes the Polar customer, deletes Clerk auth, finalizes local data/auth/billing state, durably enqueues any tenant cleanup, and then purges the local tombstone. Polar and Clerk deletes treat their supported already-missing states as successful retries.
