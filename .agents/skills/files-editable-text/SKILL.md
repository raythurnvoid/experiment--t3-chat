...
Every editable text file is collaborative by default. `files_nodes.collaborationEnabled` (`packages/app/convex/schema.ts`) is required: true for shared editing, false for whole-text saves, and null for folders, stored blobs, and read-only mounts. Node fields always exist; values that do not apply are null. Published `nonCollaborative` request options and snapshot fields keep their names and are translated at their write boundaries.
