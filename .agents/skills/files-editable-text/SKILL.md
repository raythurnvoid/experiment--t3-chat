...
Turning collaboration on also drops every content proposal on the file, like turning it off does (`files_pending_updates_db_drop_content_for_node` in the final mutation). Those proposals were built from the saved text, and the new document replaces it. A proposal that also moves or deletes the file keeps that part. The ON confirmation says "Text changes waiting for review are deleted."
