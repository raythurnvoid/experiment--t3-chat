...
description: Spec for editable text files and their Yjs shape system — the content type policy, the stored `textKind`, the read-side shape guards, the write doors, the collaborative/non-collaborative flag and its two toggles, the size limits, the four durable refusal markers, and the operator repair path. Use when changing the content type helpers or editor maps in `packages/app/shared/files.ts`, the shape guards or update scans in `packages/app/shared/files-yjs.ts` / `files-tiptap.ts`, the Yjs write doors or materialization markers in `packages/app/convex/files_nodes.ts` / `files_nodes_content.ts`, the plain-text chunker, or upload text conversion in `packages/app/convex/r2.ts`.
---
...
Every editable text file is collaborative by default. `files_nodes.collaborationEnabled` (`packages/app/convex/schema.ts`) is required: true for shared editing, false for whole-text saves, and null for folders, stored blobs, and read-only mounts. Node fields always exist; values that do not apply are null. Published `nonCollaborative` request options keep their names and are translated at their write boundaries. Snapshots use the positive boolean `collaborationEnabled`; stored-byte snapshots use false.
...
`textKind` stays set on a non-collaborative file. It still decides Markdown versus plain text for the chunker and the editor choice, and turning collaboration back on needs it to rebuild the right document shape.
...
An agent Bash copy carries the source's content, content type, and document shape, whatever the destination's name says. A text copy over an existing text file keeps the destination's collaboration mode. A new copy, or a text copy over a stored file, inherits the source's mode. The destination keeps its id, permissions, custom metadata, and history. The copy is staged as a whole-file replacement (`files_pending_updates.pendingReplacement`, see `../files-agent-pending-updates/SKILL.md`) and reviewed as a whole; the replaced content stays in history with its own type. Every destination gets a pending copy, whatever its collaboration mode. Text is normalized once (one leading BOM removed, line endings to LF). A plain text source stays byte-exact after that, and a Markdown source is re-serialized by the rich text document it builds. `mv -f` is not a copy: it is a structural replace-move that keeps the source's identity and archives the occupant when accepted.

Human clipboard Copy creates saved files through `files_transfer` and `files_nodes_content`.
Each file pins its saved source version and metadata before reading bytes. Saved Yjs edits are
included only through that version's sequence. Retries never switch to later content or metadata.
The new file keeps the source's content type, text shape, collaboration mode, and custom metadata.
It gets new content, asset, and version docs, plus new Yjs docs when collaboration is on.
Once the independent content and Yjs assets are sealed, retries reuse those same assets. Missing
source bytes before sealing fail the copy. Publishing hands asset ownership to the saved file in
the same mutation as its content, version, billing event, and receipt.
It has no pending proposal. Comment marks, threads, and old history are not copied.
See [Files transfer runs](../files-explorer-tree/references/transfer.md#copied-content).
...
Pending whole-file copy and restore share `db_install_file_content_replacement` in `files_nodes_content.ts`. Its required `pendingContent` policy is `preserve` for restore and `drop` for copy. An accepted copy still removes all destination content proposals. When the live document has edits past its snapshot, the action keeps its latest text as a version first. Otherwise the helper uses `files_snapshots.by_asset` to add the old asset to history only if this file has no version doc for it. Replacing an existing collaborative text file with stored content refuses with `Turn collaboration off in Properties before replacing this text file with stored content.` The member must use the OFF warning and acknowledgement first. A new private copy captures its source type before its first Save.

Every version doc requires its saved `contentType`, `yjsRootKind` (rich text, plain text, or null), and boolean `collaborationEnabled`. Text keeps its saved shape and mode. Stored bytes use null shape and false collaboration, even with a text-like content type. A stored restore copies the original bytes without text parsing and gives the node null text and collaboration settings. Later node changes do not rewrite old versions. Restore and download use the saved content type; URL signing does not fetch the snapshot body.
...
- `files_node_has_editable_text_content(node)` — kind `file`, has `assetId`, has `textKind`. True for BOTH modes. This is the "is this editable text" question: reads, the editor choice, and every not-a-stored-blob fork.
...
`get_file_collaboration_cleanup_state({ membershipId, nodeId })` returns null when the node is unavailable and otherwise a boolean: true only while old history blocks enabling. Properties reads this authorized status and never sees task or sequence ids. An asset-only task does not show a cleanup warning. Recovery schedules outstanding task ids in bounded pages. Node and workspace deletion transfer each task's asset and late-PUT deadline to the R2 deletion ledger before removing its task or asset.

Every live writer carries the exact current `yjsLastSequenceId`. Materialization and marker workers also check that `lastSequence`, the reconstructed sequence, and the job's target sequence agree. Restore checks the id and counter it read; repair checks the id and target sequence, then derives the next `lineageGeneration` from the current sequence doc. Incremental reads also return the id, so a client never joins a snapshot from one lineage to update docs from another. Sequence numbers restart after a mode toggle, and repair rotates the exact last-sequence id even though it keeps the numeric target. The numbers alone do not identify a document lineage. Work from an older lineage must refuse or become a no-op before it changes docs or markers. Active-document cleanup carries the same exact id. OFF and replacement cleanup read their task in the same transaction. Superseded-asset cleanup stays independent because it reference-checks the asset before deleting it.
...
- A `nonCollaborative: true` create on the public `/api/v1/files/write` door. It inserts a
  ...
  This is how a plugin backend creates its owned files. Agent text creation starts as a private collaborative draft and publishes its stored mode on Save.
...
node `textKind` and `collaborationEnabled` null. There is still no lazy Yjs creation: a collaborative file gets its document eagerly.
...
- `contentShapeMismatchAt`: set when materialization finds the reconstructed document's shape does not match the node's `textKind`. Readers report a shape mismatch instead of content; the Yjs writers refuse more updates.
...
These doors create a `plain_text` node:
...
- Agent write: `create_private_node_by_path` chooses the editable type and text shape from the name, with plain text as the fallback (`files_default_text_shape_for_name`). It creates an owner-only draft with `base.kind: "new"`. The staged text must be sealed before reads, moves, copies, or Save. Save publishes the node, text, assets, history, and receipt in one transaction. A rename keeps the draft's stored type and shape.
- Producer byte output: `files_ingestion` passes a validated MIME and text shape into the private creator. It does not derive the shape again from the name. `server/files-upload-content.ts` shares upload conversion's UTF-8, NUL, normalization, text-size, document-build, and 4 MiB Yjs-state checks. The initial private batch keeps content Preparing until one mutation commits the sealed family and its ingestion receipt. Generated Markdown with over-cap frontmatter stays as exact stored bytes. This differs from normal upload conversion, which keeps its existing frontmatter marker behavior.
