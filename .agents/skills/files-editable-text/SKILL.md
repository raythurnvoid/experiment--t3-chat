...
Both toggles call `files_pending_updates_db_mark_content_for_rebase`. It sets `contentNeedsRebase: true` on each content proposal and deletes saved-sequence markers, even when the file has no proposals. It keeps the old base pointers, branch states, pending chunks, owner, contributor threads, move/delete intent, and expiry deadline. Marking does not change `updatedAt`. Review prepares one owner's proposal on the current text before further content writes. Normal reads use committed text while the proposal needs preparation. See the pending-updates skill for merging, conflicts, and stale-result checks. Whole-file copy replacement and replacement-style version restore still drop content proposals.
