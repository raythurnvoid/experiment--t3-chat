...
- Pasting, dropping, or picking an image through the attachments-bar plus button shows a removable chip; non-image files show a toast and attach nothing. Removing the chip disables an image-only send again. Attaching images never changes the composer height; extra chips scroll sideways. Arrow keys move between chip remove buttons in one tab stop, and a keyboard removal keeps focus on a neighbor chip (or the editor when none is left).
