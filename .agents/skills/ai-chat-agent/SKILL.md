...
- Read-only mirrors of external sources (e.g. a synced public GitHub repo) are exposed to Bash under `/.mounts/<name>`, separate from the user's app file tree. They are backend-only agent context, not user-visible app files. `files_MOUNT_ROOT` (`/.mounts`, in `shared/files.ts`) is the single source of truth for the prefix; `bash_is_path_under_mounts` wraps it for Bash path checks.
- All materialized mounts share ONE reserved scope (`workspaces_GLOBAL_WORKSPACE_ID` / `workspaces_GLOBAL_GITHUB_PROJECT_ID`). Each mount is a top-level folder `/<name>` within that scope; stored node paths are `/<name>/<rel>` WITHOUT a `.mounts` segment. `/.mounts` is only the bash-visible mount point.
- Reserved external-source docs are identified by the global workspace. `workspaces_GLOBAL_GITHUB_PROJECT_ID` and `users_SYSTEM_AUTHOR` are valid only inside `workspaces_GLOBAL_WORKSPACE_ID`; do not use the reserved project id or SYSTEM author with a real tenant workspace. If future mounts need workspace/project-level ownership, store them in the normal tenant workspace/project path instead of adding another virtual project sentinel.
- Bash mounts one reserved-scope `bash_DbFilesFs` (`externalMountsDbFilesFs`) at `/.mounts` via `MountableFs`, which strips the `/.mounts` prefix so the FS receives the exact stored `/<name>/<rel>` path. `bash_resolve_db_files_shell_path` in `bash-utils.ts` maps each shell path against the available `bash_DbFilesRoots` (`app` plus `externalMounts`) to a `bash_DbFilesShellPathResolution` (`app | outside_db_files | external_mount | external_mounts_root`) and read commands branch on it.
...
- `execute_code` cannot access read-only external mounts under `/.mounts`: those docs live in the reserved `GLOBAL`/`GITHUB` scope, while public API grants are tenant-scoped to the current workspace/project.
...
- Public file routes are scoped to real tenant workspace/project ids. They do not authorize reserved `GLOBAL`/`GITHUB` mount docs, even when the requested path looks like `/.mounts/<name>` or the stored mount path `/<name>/<rel>`.
...
- Read-only external mounts skip pending-update lookups entirely because reserved-scope files are not editable and never have pending docs.
