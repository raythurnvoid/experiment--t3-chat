...
- `bash` mounts app files at the current project path `/home/cloud-usr/w/{workspaceName}/{projectName}`, blocks file writes there, allows Agent-mode folder creation through `mkdir`, and provides per-invocation scratch space at `/tmp`.
...
- `/home/cloud-usr` is the bash home directory, `/home/cloud-usr/w` is the app mount, and `/home/cloud-usr/w/{workspaceName}/{projectName}` is the current project path.
- Bash command behavior is performance-first because app files are a Convex-backed virtual filesystem, not POSIX files. Match native command shape where practical, but prefer DB indexes, then Convex `.filter()` when an index cannot express the condition, and use JavaScript filtering/sorting only as a last resort with an explicit reason.
...
- `ls -t` / `ls -rt` with no path list the whole project by update time. `ls -t <dir>` / `ls -rt <dir>` list that directory's immediate children by update time. After `cd <dir>`, use `ls -t --limit N .` for that folder's recent immediate children; bare `ls -t` is still project-wide. `ls -Rt <dir>` is unsupported.
...
- Use `find -name QUERY` or `find --path-query QUERY` only for DB-backed path/name word search; `find -name` is case-insensitive like `-iname`. Prefer `--path-query QUERY` for natural "path/name contains QUERY" requests; pass a plain token such as `readme`, not `*readme*`. For regex path requests, say regex is unsupported and use token search when a plain token is obvious; do not summarize successful `--path-query` output as native glob/regex syntax. Use `find <dir> -maxdepth 1 -name QUERY` for DB-backed immediate-child path search under one directory. Use `find <path> --extension md -type f` for exact indexed extension search. Simple `find -name '*.md'` and `find <dir>/*.md` are accepted as extension-search recovery, not general glob support. `find` searches paths/names only, not file content. When asked for files under a folder, include `-type f`; when asked for folders, include `-type d`. General glob/regex patterns are unsupported.
- Use the custom `search [--limit N] [--cursor CURSOR] <content terms...>` command inside `bash` for full-text content search. Pass one distinctive word or a few plain terms that should appear in the document body; the text index splits on whitespace/punctuation, ignores case, relevance-ranks matches, and prefix-matches the final term. It is implemented with Convex full-text search, but it is not regex/glob/exact grep or path/name search. Do not pass app paths as positional operands, do not use it as a pipeline filter, and do not treat it as substring grep. If cwd is inside the app tree, bare `search` scopes to that cwd; use `search --path <folder> <content terms>` to choose another folder. Broad scopes with common terms can be heavier. For requests like "where does X appear" or "which files mention X", run `search` first; do not substitute `find`. For recursive grep or `grep -R` wording over a folder, use `search --path <folder> <content terms>` directly.
- `grep [-i] PATTERN <file>` scans one exact app file with substring semantics and supports `-c`, `-l`, `-v`, and `-A`/`-B`/`-C N` context. Recursive or multi-file grep is unsupported; use `search` instead.
...
- `cat`, `head`, `tail`, `wc`, and `stat` read exact app paths only. `wc` supports `-l`, `-w`, `-c`, and `-m`; line/word/character counts may be lower bounds for large files. `cat`, `head`, `tail`, and `wc` may show an advisory for uploaded/unreadable source files; that advisory is not file content.
...
- Runs a curated Just Bash command surface against the current project path `/home/cloud-usr/w/{workspaceName}/{projectName}`.
- Never exposes or runs against the host filesystem.
- Starts at the current project path `/home/cloud-usr/w/{workspaceName}/{projectName}` for new chat threads.
...
- Supports `grep [-i] PATTERN <file>` for one exact app file through a bounded substring scan. Recursive or multi-file grep prints guidance to use indexed `search`.
...
- TODO: if more common glob mistakes appear in live eval, auto-fix only those that can map to a DB-backed query. Do not add JavaScript suffix filtering after pagination.
...
8. New generation uses Bash `search` for full-text content search and Bash `find` for path discovery; legacy `grep_files` / `glob_files` are validation-only surfaces.
