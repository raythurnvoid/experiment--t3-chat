...
- R2 finalization classifies every upload from the node NAME and converts editable text uploads (all 20 extensions) into editable documents in the host. Uploads that stay stored blobs — media, unknown types, and editable-text uploads whose conversion failed deterministically — dispatch `files.upload.completed` to eligible installed plugins; only a successful conversion suppresses the event.
...
- Requires an existing editable text file — Markdown or any of the 19 plain-text extensions. A classifier pre-check refuses a non-editable file type with a message that names the file class, not "File not found".
...
- Works on any file kind, uploads and binaries included, because metadata is not part of the file's text.
- Read the current map with Bash `meta get <file>`; find files by key with Bash `meta search --where '{"exists":"metadata.<key>"}'`.
...
- The public file HTTP routes resolve either a user API credential or a private public API grant token through the public API verifier, enforce expiry/revocation, file scope, active membership, and optional grant path prefix, then call `internal.files_nodes.list_subtree` or `internal.files_nodes.get_file_last_available_text_content_by_path`; public API grant reads preserve the current user's pending `unstaged` branch overlay.
...
- `read_file` goes through `get_file_last_available_text_content_by_path`.
...
- Bash `search` queries the unified `files_plain_text_chunks` full-text search docs. Pending docs are user-scoped inside that table, other users' pending chunks are filtered out, and stale committed chunk hits are hidden only for files the acting user has pending edits on. Exact text reads and regex scans use unified `files_text_chunks`; the old separate search and pending chunk tables no longer exist.
- Bash `meta search` queries unified `files_metadata_docs` with the same current-user pending overlay rule as Bash `search`. That overlay only concerns `frontmatter.*` docs, because `metadata.*` docs are always committed: metadata is stored next to the file, so a pending content proposal cannot change it. Other users' pending metadata is invisible, and stale committed metadata is hidden for files the acting user has pending edits on.
