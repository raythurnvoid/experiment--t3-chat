...
- Reserved external-source docs are identified by the global organization. `organizations_GLOBAL_GITHUB_WORKSPACE_ID` and `users_SYSTEM_AUTHOR` are valid only inside `organizations_GLOBAL_ORGANIZATION_ID`; do not use the reserved workspace id or SYSTEM author with a real tenant organization. If future mounts need organization/workspace-level ownership, store them in the normal tenant organization/workspace path instead of adding another virtual workspace sentinel.
...
- `execute_code` cannot access read-only external mounts under `/.mounts`: those docs live in the reserved `GLOBAL`/`GITHUB` scope, while public API grants are tenant-scoped to the current organization/workspace.
...
- Public file routes are scoped to real tenant organization/workspace ids. They do not authorize reserved `GLOBAL`/`GITHUB` mount docs, even when the requested path looks like `/.mounts/<name>` or the stored mount path `/<name>/<rel>`.
