...
- **Notifications:** invite notifications are in-app only in v1; no outbound email is sent. Backend notification listing only returns invites whose workspace/project still exists and whose recipient still has active project membership. Workspace member removal, project deletion, and workspace deletion should delete affected invite notifications so unavailable invites disappear instead of rendering a stale dismissal row.
