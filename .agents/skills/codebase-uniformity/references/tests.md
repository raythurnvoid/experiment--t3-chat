...
- The `browser` project runs the React Compiler with the same settings as `vite.config.ts`, so it runs the same compiled code as the app. Keep the two configs in sync. A compiled component keeps its output when its props do not change, so `rerender` with the same props does not reach a child. When a mocked hook must change its value, let it subscribe with `useSyncExternalStore` and notify it inside `act`, like the thread query mock in `ai-chat-controller.browser.test.tsx`.
