...
- If `getStatus` shows no progress for minutes while `inProgress` stays true, check the scheduler before you wait longer. Another feature can flood it with its own jobs, and then the component's next batch waits behind them. Seen 2026-09-24: private-cleanup jobs ran about 18 times per second, and a backfill stopped after its first batch. Cancel the run with `convex run --component migrations lib:cancel`. Then call the named migration yourself, one batch per call: `{ oneBatchOnly: true, dryRun: false, cursor, batchSize }`. Pass the returned `continueCursor` to the next call until `isDone`. Start Node on `node_modules/convex/bin/main.js` with `execFileSync`, not through a shell, because the cursor is JSON with quotes in it. Audit the result the same way as a normal run.
