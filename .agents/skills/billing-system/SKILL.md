...
- Keep the token-pricing switch local to `compute_token_usage_cost_cents` in `ai_chat.ts`. Do not recreate a shared entitlements module, exported pricing type, or helper for the current pricing table. DeepSeek V4.1 Flash bills the dollar cost OpenRouter reports on each step. If a step has no cost, that step uses the DeepSeek host list price: $0.30 input and $1.20 output per 1M tokens.
...
File saves ([yjs_push_update](../../../packages/app/convex/files_nodes.ts), [save_file_pending_update_in_db](../../../packages/app/convex/files_pending_updates.ts), the non-collaborative save door [replace_file_content](../../../packages/app/convex/files_nodes_content.ts), the collaboration-off accept door [save_file_pending_update_non_collaborative_in_db](../../../packages/app/convex/files_pending_updates.ts), and snapshot restore through [restore_snapshot_r2](../../../packages/app/convex/files_nodes_content.ts) / [restore_snapshot](../../../packages/app/convex/files_nodes_content.ts)) fail fast before the yjs push, the content replacement, or the restore write:
...
The last id part is called `version` because the two save paths name a version differently. A collaborative save uses `<yjsLastSequenceId>:<newSequence>`, the id of the lineage doc plus the new Yjs sequence. The sequence alone is not unique: turning collaboration off and on again creates a new lineage doc that starts at sequence 0, so the next save would repeat an id the file already used (verified in the browser on 2026-09-05, before the lineage id was added), and Polar drops an event whose `externalId` it has already seen (the SDK's `EventsIngestResponse.inserted` counts only new ids). A save on a file with collaboration off, from the member's editor or from accepting an agent proposal, has no sequence, so it uses the id of the version snapshot asset it just wrote. The part only has to be unique per save, so both work.
...
The cloud browser ([files_browser.ts](../../../packages/app/convex/files_browser.ts)) is billed by time. Both modes follow the same rules: file mode (show a workspace file) and web mode (visit public addresses).

1. **Plan check at start.** `create_starting_browser_session` (file) and `create_starting_web_browser_session` (web) call `browser_db_check_payer`. It picks the payer with `billing_pick_billed_user_id` and calls `billing_db_check_paid_plan`. When the payer is on `Free`, the start returns `_nay` `Plan required` with `data: { code: "plan_required" }`. In an owner-billed organization the owner's plan decides, not the member's. A reattach to a live session skips the check.
2. **Payer frozen at start.** The session doc stores `billedUserId` and `billing: { state: "pending" }`. A later ownership or billing-mode change does not move the charge.
3. **Receipt from the runner.** When the runner closes a session, it returns a usage receipt `{ providerAcquiredAt, endedAt, reason }`. It returns the same receipt from its status route once the session is gone (`alive: false, closing: false, usage`).
4. **Settle once.** `settle_browser_usage` bills `ceil(billedMs / 60_000) * 0.3` cents (`BROWSER_COST_CENTS_PER_MINUTE`), rounded to tenths of a cent. `billedMs` is `endedAt - providerAcquiredAt`. It emits one `billing_event("browser_usage")` through `billing_ingest_events` and marks the doc `settled`. A settled doc is never billed again, so End and the cron can race safely.
5. **Event id.** `composite_id("billing", "browser_usage", billedUserId, actorUserId, organizationId, workspaceId, sessionId)`. One session is one event. `externalCustomerId` is `billedUserId` and `externalMemberId` is the session owner. `metadata` carries `amount`, `actorUserId`, `billedUserId`, `organizationId`, `workspaceId`, `sessionId`, `mode`, and `billedMs`.
6. **Polar meter.** `browser_usage` needs no Polar change. `ingest_events` sends it as the single `press_usage_event` meter event, like every other event, with `browser_usage` in `metadata.name`. No startup or setup check verifies the Polar meter.

Who calls settle:

- A human End (`end_browser`), the server's own close (`end_browser_session_internal`, for example after access is lost), and a start whose commit was refused (`start_browser` / `start_web_browser` close the orphan runner session at once) settle with the close reply's receipt. The status refresh (`files_browser_refresh_session` in `server/files-browser.ts`) settles when the runner says the session is gone and not closing.
- The cron `settle browser usage` runs every 5 minutes (`settle_pending_browser_usage`). It first closes live web sessions whose owner lost the membership or `workspace.browser.use` (`close_web_browser_sessions_without_access`). It lists pending docs that are closed, or past their total deadline plus the receipt wait (`list_browser_sessions_to_settle`, 50 per run). For each one it asks the runner for status. It settles only when the runner says the session is gone and not closing. A doc that never got a runner session settles at 0.

Edge cases:

- **Missing receipt.** Settle waits `BROWSER_USAGE_RECEIPT_WAIT_MS` (10 minutes) after the session ended. If there is still no receipt, the runner lost it. The session settles at 0 and logs `browser_usage_missing`. We do not guess a time.
- **Closing.** While the runner is still closing the session, nothing settles, and the refresh path returns `Browser is closing`.
- **Zero amount.** A 0-cent session emits no event but is still marked settled.
- **Deleted payer.** If the billed user doc is gone, no event is emitted.
- **Deleted session doc.** A pending session doc deleted before it settles is never billed. A workspace or organization purge and the data-only reset (`hard_delete_user_data`) delete session docs without closing the runner browser; its idle alarm ends it. Account deletion instead closes each live browser at the request (`files_browser_db_schedule_user_deletion`), and that close settles it.

The browser has no credit check, only the plan check. Paid plans can go negative, the same as other usage. The daily start brakes are safety limits, not billing; see `../quotas/SKILL.md`.
...
- **Planned: remove the plugin document-slot cap and bill per write instead** (product decision, 2026-08-25, not implemented yet). The plan-driven slot ceiling (Free 10,000 / paid 100,000 in `plugins_data.ts`) is a temporary rail, not the final product rule. It punishes exactly the users we want: people who live in the app for a long time naturally accumulate data, and a ceiling forces them to delete things that are not garbage. The end state is: no ceiling at all, and the user pays for what they write (a charge per write, not a daily charge on stored bytes — that model was built and then removed on 2026-08-25 because it added a cron, an accrual table, and payer questions for almost no money). The known abuse case for uncapped writes is a user who writes a huge amount very fast on a card that cannot pay. The planned answer is threshold billing: when the amount due crosses a level (for example every $100), charge the user immediately instead of waiting for the period end. Do not implement any of this yet; when it happens, remove `PLAN_MAX_DOCUMENT_SLOTS` and the cap tests together with adding the per-write charge, and keep the per-member share and byte ceilings only if they still earn their place then.
