...
**Publish from the detail page when the QA Edge profile IS signed in as the claim owner (verified 2026-09-09, Council 0.2.14).** Open `/w/<org>/<ws>/plugins/<name>` on an owned tab and wait until the page lists versions (the hidden `Publish reviewed commit` button of the mounted-while-closed dialog shows up first; wait for `Publish <owner>/<repo>` and `Uninstall`). Click `Publish <owner>/<repo>` (regex, not `exact`), wait until the dialog text carries `Current default-branch HEAD<sha>`, fill the `Reviewed commit SHA` textbox with that exact SHA, click `Publish reviewed commit`. The dialog shows `Publishing...` for about two minutes while the review runs and then closes with no toast; prove the publish with a registry readback (`plugins_versions` row for the new version: `sourceStatus ready`, `reviewStatus passed`, `sourceCommitSha`), then the detail page shows `Update`. The update consent modal lists the FULL capability set and a service-account grant picker (`Workspace`, `Can view`, `Add reviewed grant`); an ordinary update needs no grant, so leave the picker alone and click `Accept and update`.

**A plugin's read-only files need "Can manage" on its service account.** A service creating a file with `readOnly` (`create-target` or `files/write` with `access.readOnly`) is refused with `403 Permission denied` unless the installation's service account holds `content.permissions.manage` at the destination; the workspace-root grant is the one that covers nested folders. The 2026-09-07 service-account backfill granted only "Can write", so every plugin that locks its files was refused after that date until an owner raised the level (Service accounts page, or `access_control.set_service_account_grant` with `resource: { kind: "workspace" }, level: "manage"` from page context). Check the grant before blaming the plugin.
...
**Publish confirmation dialog anchors (fresh build, verified 2026-08-30 on the redeployed Pages host).** The detail-page Publish button's accessible name is `Publish <owner>/<repo>` (aria-label), so `getByRole("button", { name: "Publish", exact: true })` matches nothing — use the full name or a non-exact match. Clicking it opens `.PluginsPublishSessionProvider` (the dialog is rendered by the session provider in `src/components/plugins-publish-session.tsx`, so the class carries the provider's name, not the button's): heading `Publish <owner>/<repo>`, a read-only "Current default-branch HEAD" SHA (from `get_publish_candidate_head`), a `Reviewed commit SHA` textbox, `Cancel`, and `Publish reviewed commit`. The modal is MOUNTED WHILE CLOSED, so its `Publish reviewed commit` button appears (hidden) in DOM dumps before any click — check visibility before concluding the dialog is open. `Cancel` closes it without any mutation; the preflight HEAD read is safe. **The QA Edge profile cannot reach this button at all**: it is signed in anonymously, and the publisher route answers anonymous visitors with the `.RoutePluginsPublisherSignIn` gate, so there are no repository cards. Every claim in the dev database belongs to the user's own Clerk account, which an agent must not sign into. To drive the dialog, sign in as a `+clerk_test` account in a scratch browser (`clerk-test-accounts.md`), claim any unclaimed public repository URL — `claim_repository` checks no GitHub ownership — drive the dialog, then Remove the claim. Space the claim and the remove about 15 s apart for the `plugins_manage` bucket. Also: on the Pages host every deep-link document request logs `Failed to load resource ... 404` in the console — that is GitHub Pages' 404.html SPA fallback booting the app, not an app error.
...
**Checking where keyboard focus lands after a publisher card disappears (verified 2026-09-04, dev host, `qa.perm.owner+clerk_test` in a scratch Chrome).** Two flows, both provable without publishing anything real.

_Removal._ Tab from `Publish <owner>/<repo>` to `Remove claim on <owner>/<repo>`, press Enter, then wait for the card node itself instead of a label: `waitForSelector(".RoutePluginsPublisherUnpublishedCard", { state: "detached" })`. Read `document.activeElement` right after — it must be the claim field (`input[placeholder="https://github.com/owner/plugin-repo"]`). For the "do not steal" case, park focus on the `Claim` button while the removal is still in flight (the button needs a non-empty URL to be enabled); focus must stay there. The route now repairs this from a ref cleanup on the card, so it works even though the Convex subscription, not the click, removes the card.

_Publish replacing the card while the dialog is open._ A bogus repository URL is useless here: `get_publish_candidate_head` fails, the app toasts and CLOSES the session, so no dialog stays open. Claim a real unclaimed public repository instead (`https://github.com/octocat/Hello-World`, `.../Spoon-Knife`), open `Publish <owner>/<repo>` and let it reach the `Reviewed commit SHA` step, then flip the card from the CLI with the fixture recipe below — `plugins:upsert_plugin` while the dialog is already open, then `plugins:finalize_plugin_version`. Only `sourceStatus: "ready"` decides `readyVersions`, so `reviewId: null` with `reviewStatus: "passed"` is enough and no review row is needed. The card behind the dialog turns into a gallery card; press `Escape` and read `document.activeElement`. It must be the claim field. If it is the `qa-browser/home workspace content` region instead, the route lost its `setRouteFocusTarget` registration — that region is the workspace-level fallback in `plugins-publish-session.tsx`.

Cleanup for both: `plugins:hard_delete_plugin_from_registry` until `done: true` (it also removes the publisher claim), then `plugins:clear_plugin_registry_deletion_fence` per name — the hard delete leaves the fence row behind on purpose and a leftover fence blocks that plugin name forever. Check `plugins_publisher_repositories` and `plugins_registry_deletion_fences` are back to their pre-run rows.
