...
- **Tooltips / hovercards** — hovercards need a real pointer position change (the Ariakit entries under Interaction Discipline). Tooltips and `MyPopover` are native (`native-popovers`); see the `MyTooltip` and `MyPopover` entries there.
...
- The Vitest browser project loads the app's Tailwind Vite plugin so style tests include generated utilities. Import `app.css` before component styles to establish the app's CSS layer order. When a regression depends on a library utility, verify that the test fails with the relevant fix removed; also check the live app. The happy-dom `src` project and the browser project both run the React Compiler with the app's settings (see `packages/app/vitest.config.ts`), so a bug that only happens in compiled code, like a list the compiler keeps stale, shows up in the main tests too. A compiled parent keeps its output when its props do not change, so `rerender` with the same props does not reach a child: let a mocked hook subscribe with `useSyncExternalStore` and notify it inside `act` (see the thread query mock in `src/hooks/ai-chat-controller.browser.test.tsx`). The snapshot value has to be part of the value the component reads. An unused counter lets the compiler keep the old return.
...
- Do not use Vite Plus package-filtered execution for Playwriter; it changes the cwd to `packages/app`. Keep the personal task folder as cwd and pass absolute helper paths.
...
- **Write runner and generator files with the editor's file-write tool, not with a shell heredoc from the agent's Bash tool.** Verified 2026-09-06 in Claude Code: a `cat > gen.mjs <<'EOF'` heredoc whose JS template literal held `\\n` landed on disk as `\n`, so the file it generated carried a real newline inside a string and failed to parse with `Unterminated string`. The quoted heredoc is not at fault; the agent tool rewrote the backslashes before bash saw the command, and a `grep 'foo\\n'` from the same tool matched nothing for the same reason. Keep heredocs for text without backslashes.
...
- **Starting a relay by hand on playwriter 0.5.0 needs `--host 127.0.0.1`, and "already running" can be true while Windows sees no listener.** Bare `playwriter serve` now refuses with `Authentication token is required when binding to a public host` — it treats the default bind as public and wants `--token`. With `--host 127.0.0.1` you get one of two answers, and both are informative: `Playwriter server is already running on port 19988`, or a `net` listen error. Check `netstat -ano | grep 19988` before believing either. **A WSL relay owns the port without appearing as a Windows LISTENING socket** — you see only client sockets in `TIME_WAIT`/`ESTABLISHED` — so "already running" plus no Windows listener means WSL has it, the same state the CWD column reports. That is not a broken relay and not something to fix; it is somebody else's relay. Observed repeatedly on 2026-09-04, when the relay changed owner five times in one session.
- **Relay churn destroys sessions mid-run, so write runners that do not depend on a stored `state.page`.** When the relay is replaced, `session list` restarts its ids and your session is gone with everything on its `state`; the next command fails with `Session N not found`, `fetch failed`, or a hang. The cheap defence is to open each runner with `const page = context.pages().find((p) => p.url().includes("localhost:5173")) ?? (await context.newPage()); state.page = page;` — then recovery is `session new` plus a re-run, with no re-binding step and nothing lost. This is the same lesson as the `state`-survival note above, applied to the runner rather than to your notes.
...
- **On `/files`, `auditAccessibility` lists most of the sidebar tree as `blockedHitTargets`, and almost none of it is a bug.** A body-wide audit of that route reported 341 controls with **zero** unlabeled, but around thirty entries under `blockedHitTargets`: `Expand folder <name>`, `Add file to <name>`, `Add folder to <name>` and `More actions for <name>`, repeated for every folder. Their `topAtCenter` is `FilesSidebarTopSection` or `MyInputArea FilesSearchInput-area`. Those two are the sidebar's sticky top bar and its search box, and the listed rows are simply scrolled up underneath them. The audit hit-tests an element's geometric centre wherever it currently sits, so any row above the visible area of a scrolling list reports as covered. Scroll the row into view first (`computer scroll_to`, or the `.FilesSidebar-content.app-scrollable` container) and re-audit before believing a single one of them, and do not report them as app defects. The same route also reports three `smallTargets`: the two 8px splitters (`Resize files sidebar`, `Resize comments and agent sidebar`) and `Open TanStack Router Devtools`, which ships only in dev. Verified 2026-09-14.
  Re-checked 2026-09-25 on `/w/qa-browser/home/files?nodeId=root` at 1440x900 (86 controls). Harness 0.6.6 no longer reports clipped toolbar buttons or sr-only disclosures (see the half-scrolled entry below), so blocked went from 15 to 8. All 8 left are by design. The splitters are `8x9999` on purpose: the control is 9999px tall, overlaps its neighbours by about 3px, and so covers one edge of the tree row, the editor, and the sidebar tab panel. The splitter is a real control: Tab reaches it, arrow keys change `aria-valuenow`, and its hover grip is 24x52. Each folder explorer `Open <name>` overlay link has its right corners under the actions cell. That is the row pattern in `AGENTS.md`: the actions column sits above the primary action. `click({ trial: true })` passes on all of them.
...
- `auditAccessibility` also returns `controlCount: 0` for a region behind an open modal. Ariakit marks `.RootLayout` `inert` while a modal is open, and the audit skips inert controls. So the audit is right, but it looks like a clean region. A review modal can stay open after you start a Discard or Save: `.FilesPendingReviewModal` stays up while the run waits in its queue. Check `el.closest("[inert]")` on your region first. Then either audit the modal by its class, or close it (`Hide` does not stop the run) and audit again. Verified 2026-09-24 on the Pending panel: 0 controls with the modal open, 8 after `Hide`.
...
- `latestLogs({ sinceLastCall: true })` can return `[]` right after a new line arrived. Seen 2026-09-05: a Convex mutation `console.log` (forwarded to the browser as `[CONVEX M(files_nodes:yjs_push_update)] [LOG] ...`) was missing from the "since last call" read, and `getLatestLogs({ page: state.page, search: /<marker>/ })` over the whole buffer then listed it. An empty "since" read is not proof that nothing happened. To assert a specific line, put a unique marker in it and search the whole buffer for that marker.
...
- **A route handler outlives the runner that installed it.** A failed runner can leave its request delay in place, so the next navigation times out. Keep the exact handler reference and remove only your own route with `page.unroute(pattern, handler)` (or the matching context method). Use page-scoped routes in an owned tab when other tasks share the browser. Clean up in `finally`; do not remove every route from the shared context. Observed while holding plugin asset responses on 2026-09-04.
...
- **A relocated popup can leave a stale Playwriter handle.** A Files browser popup appeared in `context.pages()`, but `snapshot` on the first session's popup handle timed out. Rebinding the exact existing popup URL through a second owned Playwriter session worked: locators read the live panel and clicked Resume normally. If a locator reads the control but `click` stalls while waiting for stability, click the app's `Focus popout` button in the opener, then retry the native popup control. This restored a native `Dock in Files` click and kept human input working after docking. The popup still had the plain `Press` title from `index.html`, which did not mean the app failed to load. Try these steps before opening a replacement tab or testing synthetic transfer messages. Verified 2026-09-19. Do not close another task's tabs or reset the shared relay.
...
- **The QA Edge profile is a different user on each origin, so `/w/personal/home` is a different workspace on each host.** Sessions live per origin: on `http://localhost:5173` the profile is the anonymous QA user (org owner of the QA `personal/home` where Chitchat and Council are installed), while on `https://raythurnvoid.github.io` the same profile holds the Clerk user's session, whose `personal/home` is another org with its own installs. A plugin page that works on the dev host answers `This plugin page is not available.` on the Pages host, and the sidebar lists different plugins. It is not a deploy regression: read the sidebar's plugin list (or `plugins_workspace_installations` for that org) and open the workspace that has the install — the Clerk user owns `chitchat-qa/home` for Chitchat. Never sign in or out to "fix" it. Verified 2026-09-02.
- **The Pages host needs its base path, and the identity there can be anonymous.** Two follow-ups to the entry above, both hit 2026-09-02 in the afternoon. The deployed app lives under `https://raythurnvoid.github.io/experiment--t3-chat/` (the workflow builds with `--base /experiment--t3-chat/`), so a URL without that segment answers GitHub's own "There isn't a GitHub Pages site here" 404 page, which looks like a failed deploy and is not one. And the profile's Clerk session on that origin can be gone: the same afternoon the app there rendered `Anonymous user m5759db9…` and `You do not have access to this organization/workspace.` for `chitchat-qa/home`. Read the sidebar's user line first and plan the check around the identity you actually have. Do not sign in to get the Clerk one back.
- **When a Pages-host check really needs a signed-in workspace, use an isolated scratch browser, not the user's profile.** The two entries above end in a dead end: the QA Edge profile can be anonymous on the Pages host, and you may not sign it in. The way through is the scratch browser from `clerk-test-accounts.md`: launch Chrome for Testing on `--remote-debugging-port=9223` with a temp `--user-data-dir`, `session new --direct 127.0.0.1:9223`, `goto` the Pages base URL, check `window.Clerk.publishableKey` starts with `pk_test_`, then sign in as a seeded `+clerk_test` account. The Pages origin runs the same dev Clerk instance and the same Convex deployment as localhost, so that account brings its real workspaces with it. `qa.perm.owner+clerk_test@example.com` owns `qa-browser/home`, which has Chitchat installed with seeded channels. Sign out, close the browser, delete the profile folder and the session when done. Verified 2026-09-03.

- **A tab whose URL reads `""` is not a usable blank tab: open a new one.** `bindOpenTab` answers `No matching Playwriter-enabled tab found` when the profile has no app tab, which is the normal state after the user closes their tabs. `context.pages()` can still list entries whose `url()` is the empty string, and reusing one fails the first `goto` with `page.goto: Frame has been detached`, followed by a relay-side `Assertion failed: !(handle->flags & UV_HANDLE_CLOSING)`. Do not treat that as a broken relay or retry the same page. Call `await context.newPage()` and `goto` the route on the fresh page. Verified 2026-09-16.
- **An existing session can be on the wrong Edge profile while its CWD looks right.** After the machine restarts, `session list` can show a session whose CWD is this repo and whose PROFILE is another agent's work profile. Reusing it drives QA in the wrong browser and the wrong signed-in identity, and nothing in the output says so. Read the PROFILE column of `session list` against the profile column of `browser list` before reusing a session, not the CWD or the session number, and create a new session on the right key instead of reusing a wrong one. Verified 2026-09-16.

## Interaction Discipline

- Always bind `state.page` to the target tab before acting.
- Playwriter sessions in the same browser profile share the foreground tab. Coordinate native clicks, because switching tabs can dismiss another session's open menu.
...
- With Playwriter 0.7.0, an `-e` script that starts with a newline runs nothing. It prints nothing and exits 0, so it looks like a quiet success (2026-09-25, from Git Bash). Start the script on the same line as `-e`, or put it in a file and pass `-f <file>`. When a call prints no `Console output:` at all, suspect this before you suspect the page.
- On Windows PowerShell, piping `playwriter` output into `Select-Object -Last N` can crash the CLI with `Assertion failed: !(handle->flags & UV_HANDLE_CLOSING)` before later steps run (2026-09-24). Redirect to a file (`> out.txt 2>&1`), then read the file.
- `locator(".FileEditorSidebarPending").screenshot()` can hang on "waiting for element to be stable" (Pending panel, 2026-09-24). Read the panel's `getBoundingClientRect()` from page context and pass it as `clip` to `page.screenshot({ clip, animations: "disabled" })` instead.
...
- `MyTooltip` is the native tooltip from `native-popovers`, not Ariakit. The open tip is `[role="tooltip"].np-Tooltip.MyTooltipContent` inside a `.np-TooltipPositioner` with `popover="manual"`. There is no portal: the positioner stays next to its trigger in the DOM, and the browser shows it in the top layer. A closed tip is removed from the DOM. A plain `mouse.move` onto the trigger opens it after the delay (500ms by default, 2000ms on sidebar tree rows); no 3px nudge is needed.
- App tooltips are not interactive: `MyTooltipContent` defaults to `interactive={false}`, so the tip and its positioner have `pointer-events: none`. An open tip never covers the control under it. If `elementFromPoint(...)` at a button center names a tooltip, a call site passed `interactive`, or `my-tooltip.tsx` changed. Check that before working around it in Playwriter. Verified 2026-09-24: 15 hit-test points over an open tree-row tip all reached the page.
- `MyPopover` (notifications, chat jobs, rich text link and comment) is the native popover from `native-popovers`, not Ariakit. The content is `[role="dialog"].np-Popover.MyPopoverContent` inside a `.np-PopoverPositioner` with `popover="manual"`, and it has `data-open` while open. There is no portal: the positioner stays next to its trigger in the DOM, and the browser shows it in the top layer. Closed content stays in the DOM unless the call site passes `unmountOnHide`, so find the open one with `.MyPopoverContent[data-open]`, not by presence. Each dialog has a name: `getByRole("dialog", { name: "Notifications" })`, and likewise `"Background jobs"`, `"Link"`, and `"Comment"`.
- `MyMenu` and `MyContextMenu` are the native menu from `native-popovers`, not Ariakit. The open menu is `[role="menu"].np-Menu.MyMenuPopover` inside a `.np-MenuPositioner` with `popover="manual"`. `MyMenuPopover` adds no portal: the positioner stays where it renders in the DOM, and the browser shows it in the top layer. A closed menu is removed from the DOM. DOM focus stays on the `role="menu"` element. The active item is the one named by its `aria-activedescendant`, and it also has `data-active-item`, so read that, not `document.activeElement`. The menu takes its name from the element that opened it: the button (`More actions for <name>`), or a context menu trigger that has an id, like a chat tab. So `getByRole("menu", { name })` finds it. A submenu is a DOM child of its parent menu. Arrow keys skip hidden items (the tree menu's `Add file` and `Add folder` are `display: none` unless the tree is compact). Four menus use a React portal, so do not look for them next to their trigger. The files tree row menu renders into `.FilesSidebarTree-menus` right after the tree, not inside `role="tree"`, because a tree may own only its items and Headless Tree reads keys on the tree element. The agent chat tab menu and the folder explorer row menu render into `#app_hoisting_container`: a tablist may own only tabs, and the folder row reads clipboard keys with a native listener that would run before the menu. The rich text block menu renders into the editor's parent element, outside the draggable drag handle. Its `Color` and `Turn into` items are `menuitemradio`, not `menuitem`, and `aria-checked` marks the current one. The Color submenu has two groups, `Color` and `Background`, and both have a `Purple`, so scope the item to its group: `getByRole("group", { name: "Color", exact: true }).getByRole("menuitemradio", { name: "Purple", exact: true })`. The sample "A" before each color name is `aria-hidden`, so it is not part of the name, and typeahead skips it: `p` reaches `Purple`. The Color menu stays open after a click. A `Turn into` click puts the caret back in the text, and the menu closes because focus left it. In a background tab this takes about 2 s, because the browser runs animation frames slowly there. So read the new check right after the click, and reopen the menu on the changed block to put it back. The account menu is one group named by the status and the user name, like `Signed in <name>` or `Not logged in <name>` (verified 2026-09-25).
- When an Ariakit modal opens from inside an open `MyPopover` (notifications → "View review progress"), the popover stays open under the modal: the trigger keeps `aria-expanded="true"`, and the popover is inert and `visibility: hidden` until the modal closes. `checkVisibility()` ignores `visibility` unless you pass `{ visibilityProperty: true }`, so the bare call still reports the hidden popover as visible. Verified 2026-09-24.
...
- The folder explorer also **shows only the first 5 children by design** (`FILE_NODE_VIEW_FOLDER_INITIAL_VISIBLE_ITEMS_COUNT` in `file-node-view.tsx`), name-sorted, with a `Show more` button below the list. A 6th file that Convex and the sidebar both have is simply behind that button, and scrolling does not reveal it — so "the server has the file but the folder view does not list it" reads like a projection/visibility bug and is not one. Click `Show more` (or count against the sidebar tree) before filing a missing-file finding. Cost a wrong-lead investigation 2026-08-30 on a 6-file Council meeting folder whose `transcript.md` sorted last.
...
- Closing the files sidebar **unmounts** it (`closeBehavior="unmount"`). After that, `[role=treeitem]` count is 0 even on `/files`, and sidebar clicks hang. Reopen with focus+Enter on `Open files sidebar` before any tree work.
- `.FilesTransferRunModal` has two controls named `Close`: the footer ghost button and the header dismiss. Scope `button.MyButton-variant-ghost` with `hasText: /^Close$/`, or use `.nth()`. A bare `getByRole("button", { name: "Close" })` inside the modal is a strict-mode violation.
- Sidebar Cut/Copy hotkeys read **selected** rows, not the focused row. Arrow keys move focus only. A focused child with a selected parent copies the parent. The folder table uses the focused row instead. Do not treat "focus a tree row, Control+X" as a source-id proof.
- The top more-options clipboard item is always `Paste into root folder`. Do not click it on a real home tree. Toolbar Paste targets the open folder or file's parent; hidden archived destinations stay disabled and keep their folder name.
...
- The presence toggle is **not** an opt-out for the global room, but it is not purely cosmetic either (verified in-browser 2026-07-26). `MainAppSidebarPresenceControl` calls `usePresence(app_presence_GLOBAL_ROOM_ID)` **before** the enabled/disabled branch, so with the toggle off the tab still sends `presence:heartbeat` for `app_presence_global` every 10s and `presence:list` (with the room token that heartbeat mints) still returns that user as `online: true`. Check it through `list`, not `listRoom` — `listRoom` refuses the global room outright now, which says nothing about whether the user is still registered. Never treat `Disable` as "this user is no longer visible" when checking a presence leak. What the flag _does_ gate is `FileEditorPresenceSupplier` (`file-editor.tsx`), which swaps to `_Disabled`: on Disable the tab sends one `presence:disconnect` and stops all `files_nodes::<org>::<workspace>::<nodeId>` heartbeats, so per-file collaboration presence really is off.
- A `Tab` press is keyboard input, so the element it lands on shows its tip when it is a `MyTooltipTrigger`. After `evaluate(el.focus())` + `Tab`, expect the next trigger's tip (for example `Home`), not a stuck tip on the element you left. The old Ariakit stuck-open case is gone (checked 2026-09-24). Still assert the visible `[role="tooltip"]` count before a screenshot or an axe run.
...
- A tooltip opens on focus only when the trigger matches `:focus-visible`. After a mouse click, a programmatic `locator.focus()` gives focus without `:focus-visible`, so no tip appears and the check looks like a missing tooltip. Reach the trigger with `Tab`, or hover it. The reverse also happens: when the trigger was focus-visible before, a later programmatic focus (a modal giving focus back) can open its tip. Ariakit did the same.
- Pick the open tip with `[...document.querySelectorAll("[role=tooltip]")].find((t) => t.checkVisibility())`. A closed native tip is removed from the DOM, but a call site can pass `unmountOnHide={false}`, and a probe must not read a hidden one.
- Escape closes the top open layer first. With a tip open inside a popover (for example `Apply link` in the rich-text link setter), the first `Escape` closes only the tip and the second closes the popover (checked 2026-09-24).
- A native tip stays in its trigger's DOM parent, so it inherits text styles from there. `.MyTooltipContent` resets font, weight, and `white-space` for this. If a tip inside Monaco or a bold badge shows the wrong font or does not wrap, compare its computed text styles with a `div` of the same classes on `<body>`.
- To read one element's name and description, use CDP `Accessibility.getPartialAXTree` with `fetchRelatives: false` on that node (get the node id with `DOM.querySelector`). A runner that hovered once and ran three document-wide `Accessibility.queryAXTree` calls on `/files` took more than 5 s, which is over the usual `--timeout 5000` (seen 2026-09-24).
...
- `snapshot({ locator })` can also return another tab for a top-level locator. Verified with Playwriter 0.5.0 on the Files route on 2026-09-09. Use `snapshot({ page: state.page })` for the page and `getCleanHTML({ locator })` for a scoped region. Confirm the URL before trusting either result.
...
- A content discard keeps accepted hunks. To reset a disposable collaboration-off fixture completely, read its committed text, save that same text through `files_nodes_content.replace_file_content`, then call `discard_file_pending_content` with the current proposal id. The save gives the file a new asset id, so the stale discard removes all its content branches. A move or delete on that row still needs its own Discard action.
...
- The pinned **4.12.1** used for the room reviews, once under
  `t3-chat-+personal/+ai/council-production-room-2026-08-22/axe-install/`. That folder is gone
  (checked 2026-09-25). Install 4.12.1 again with the `snippets.md` recipe when a result has to line
  up with an earlier run. The recipe installs into `$env:TEMP\axe-install`. To keep scratch files in
  your task's `+ai` folder, set `$axeDir` to that folder and pass the same path to `addScriptTag`.
...
- `Open file snapshots` can be reported as a blocked hit target on the editor toolbar (harness 0.6.5 and older; 0.6.6 skips points the toolbar clips). That is toolbar **overflow**, not an overlay: check `scrollWidth > clientWidth` on `.FileNodeViewToolbar` before filing it — with a narrow panel the trailing buttons clip under the Comments tab strip (a recorded design question, not a broken control). The rich text `Add link` and `Comment` buttons clip the same way: at 1440x900 with the file sidebar open, `Add link` sits at x≈813, past the toolbar's right edge (x≈796), and a mouse click at its box opens a sidebar tab. Call `button.scrollIntoView({ inline: "nearest" })` first, then click its new box. Verified 2026-09-24.
...
- After a Clerk `signOut()` in a headless scratch session, the app mints a new anonymous user within a few seconds. Delete it with the anonymous-user delete flow before deleting the session. Wait for `[data-app-ready]` and for `localStorage["app::auth::anonymous_token_user_id"]` first: a delete that runs earlier answers `Unauthenticated` and deletes nothing.
...
The recovery is not guaranteed. On 2026-09-06 an HMR update of `file-editor.tsx` printed the same
lines and left the `/files` tab with no editor until a reload. After the reload, run
`scripts/install-harness.js` and `scripts/agent-chat-helpers.js` again before the next runner:
...
- **An anonymous session cannot create an API key, and the refusal looks exactly like the stuck-submit-button hazard above.** The reserved QA Edge profile is not always signed in to Clerk — on 2026-09-01 it held an anonymous user (`Anonymous user <id>` in the account menu). `Create API key` then submits, the mutation answers `Unauthenticated`, and the dialog stays open with the word `Unauthenticated` in a `role="alert"` div at its end. There is no toast, the button is not disabled, and both submit paths (the `button[type=submit]` click and `Enter` in the name field) behave identically, so it reads as "the submit button does not work" and sends you into the line-99 fallbacks for nothing. Read the LAST ~300 characters of the dialog's `innerText` after any submit that seems to do nothing — the error is there. Do not sign in to fix it: Clerk is single-session in the user's own profile. Check the account menu button text first if a run needs a signed-in identity.
- The public-API file doors cannot be driven from the browser at all in that state, since every `/api/v1/*` call needs a key. Reaching a backend route then means the Convex CLI (`convex run <module>:<function> '<json>'`, module path without an `internal.` prefix — `internal.public_api:can_write_file_node` fails with `Function name has too few parts`), which runs against the dev deployment and is not a browser check. Say which one you did.
...
Harness 0.6.6 (2026-09-25) does this check for you. Hit testing respects overflow clipping, so at a
clipped point the control is missing from `document.elementsFromPoint(x, y)`. The audit now skips
those points. This removed six rich text toolbar buttons (`Underline` to `Table commands`) that
`.FileNodeViewToolbar` scrolls past its right edge on `/files`. Two things it still reports, on
purpose: a row scrolled up under a sticky header inside the same scroller (the row is still in the
point's stack), and any control with `pointer-events: none`, which is never in the stack. The same
version also skips the size and hit tests for a visually hidden control (`clip-path: inset(50%)` or
`clip: rect(0 0 0 0)`, the `sr-only` pattern), because its pointer target is a visible twin. It
still counts toward `unlabeled`. A temporary overlay over `Bold (Ctrl+B)` and `New folder` was still
reported as blocked at all five points, so real covers are not hidden.
...
It is not always a small corner box. On 2026-09-24, at a 1150px emulated viewport, it was 408×34 at
x 371, over the file header breadcrumbs. A real `mouse.move` onto a crumb hit the toolbar, so the
crumb's tooltip never opened. Hide it for the run with the snippet below, and put it back after.
...
Hit twice on 2026-09-06 on `/files?view=diff_editor` with a pending proposal: `locator.click()` on
`Accept all pending changes in this file`, and later `keyboard.press("Enter")` with a toolbar button
focused, left the tab's renderer at 100 % CPU, every later `page.evaluate` timed out, and Playwriter
could not close the tab. The app was fine: the same route idled 120 s alive in a new tab, and clicks
from page context worked. The relay has no `newCDPSession` (`No tab found for method
Target.attachToBrowserTarget`), so the wedged renderer cannot be paused or inspected.

What works on that toolbar: click from page context
(`document.querySelector('[aria-label="Accept all pending changes in this file"]').click()`), or
drive `page.mouse.move` / `down` / `up` on the button's rect. This is the one place where a DOM
`click()` is the recipe rather than a bypass, and only because the Playwright paths wedge; read the
result (toast, `list_files_pending_updates`, the view class) in the same call so you know the click
landed. The breadcrumb Properties dialog, the sidebar tabs, and the Pending rows still take
`locator.click()`. If a tab wedges, bind `state.page` to `await context.newPage()` and go on; do not
retry on the wedged tab.

## A `MutationObserver` can miss a toast — poll `[data-sonner-toast]` every 100 ms

On 2026-09-06 a `MutationObserver` on `body` (`childList`, `subtree`) never reported the
`Proposal discarded` toast, while a 100 ms `setInterval` that reads every `[data-sonner-toast]`
`textContent` caught it 1.4 s after the click. Start the poll before the click, collect distinct
texts, and stop it after the readback. The same poll is what makes `Changes saved` reliable on the
review Save.

## Drive the agent from a second tab when the check is about focus

The Agent panel sits in the same tab as the review, so `state.qa.send(...)` clicks the composer and
moves focus off the toolbar. A "focus moved to X when the proposal changed" check then measures your
own click. Open the same node in a second tab (`state.agentPage = await context.newPage()`), run the
agent turn there, and read `document.activeElement` in the review tab. On 2026-09-06 the
stale-to-fresh focus check only meant something once the agent turn came from the second tab.

## `app_convex.query(...)` in page context can answer from the subscription cache

`ConvexReactClient.query()` returns the current value of a query the page already subscribes to, so a
readback right after an action can show the state from before the write. For a fresh read, build a
`ConvexHttpClient` with the Clerk token and keep it on `window`:

```js
const m = await import("/src/lib/app-convex-client.ts");
const { ConvexHttpClient } = await import("/@id/convex/browser");
window.__fresh = async (ref, args) => {
	const c = new ConvexHttpClient(m.app_convex.url);
	c.setAuth(await window.Clerk.session.getToken({ template: "convex" }));
	return c.query(ref, args);
};
```
...
On 2026-09-20 there were two of them at once, and one covered the whole viewport, so a hit test on
the file header breadcrumb and on the sidebar rows reported the overlay as the blocker. The relay
injects them again after every navigation. Hide every one (`querySelectorAll`) with
`style.display = "none"` at the start of each runner, and put them back at the end of the task.
...
Dependency scanning after a lockfile change can take several minutes. During that scan even
`/@vite/client` may not answer yet. Read the Vite terminal before treating the server as stuck.
After bundling, Vite may reload more than once as it finds more dependencies. If the page stays
blank with `504 (Outdated Optimize Dep)`, reload once after bundling settles, then poll readiness
in short calls. Do not restart a live server just because the first navigation timed out.
...
## A red/green switch in a TSX file needs a full navigation, not an HMR wait

Every save of a `.tsx` file the open route imports blanks the tab (the HMR crash above). Waiting
for `[vite] hot updated` never brings the page back. After each switch, send `page.goto(<route>)`
fire-and-forget, then poll for a landmark in separate short calls (the `/files` reload pattern
above). Budget about 10 s per switch. Hit 2026-09-20 on `file-node-view.tsx`,
`files-archive-modal.tsx` and `files-sidebar.tsx`.

A switch in a shared primitive can take much longer. On 2026-09-25 each switch of `my-hovercard.tsx`
needed 40-60 s before the reloaded `/files` tab answered, and every `evaluate` in between timed out
at 5 s. The timed-out calls queue up in the session, so a later call waits behind them too. Sleep
about 30 s in the shell, then read one small status value. Before trusting the switch, fetch the
module with a cache buster and check that your change is in or out (the stale-transform entry above).

## Locator clicks time out while `page.evaluate` still answers: click by coordinates from page context

On 2026-09-20 every `locator.click()` and `page.hover()` on the files route burned the full 5 s
while `page.evaluate` answered at once. The frames were stalled, not the page. What worked: read the
element's box in page context (`scrollIntoView` first, then `getBoundingClientRect`, then
`document.elementFromPoint` on the centre to confirm nothing covers it), then `page.mouse.click(x, y)`.
Keep at most three such clicks per runner, or the 5 s budget runs out. A menu item below the fold
of a 300 px tall row menu reports "blocked by DIV" until it is scrolled into view, so do the
`scrollIntoView` every time.
...
`MyModal` names the dialog from its `MyModalHeading`. `getByRole("dialog", { name })` works. A CSS
...
While a `MyModal` dialog is open, the rest of the page is `inert`, so nothing outside it can take
focus, and `document.activeElement` is `BODY`. When the dialog closes, Ariakit first gives focus back
to the button that opened it. Whatever the app focuses next (the sidebar archive moves focus to the
...
The watcher's TypeScript check can hang for more than 18 minutes with no error, so your edits never
reach the deployment and every browser check runs old server code. A second agent's
`convex dev --once` that is stuck at the same step makes it worse. Check the watcher output for a
new `Convex functions ready!` line after each edit. If it does not come, stop only your own watcher
and start it with `vp env exec pnpm --dir packages/app exec convex dev --typecheck disable`. That
