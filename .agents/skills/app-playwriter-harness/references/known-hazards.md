...
- The Vitest browser project loads the app's Tailwind Vite plugin so style tests include generated utilities. Import `app.css` before component styles to establish the app's CSS layer order. When a regression depends on a library utility, verify that the test fails with the relevant fix removed; also check the live app. The browser project also runs the React Compiler with the app's settings (see `packages/app/vitest.config.ts`), so a bug that only happens in compiled code, like a list the compiler keeps stale, shows up there. The happy-dom `src` project does not run the compiler, so check a compiler-only bug in a browser test or in the live app. A compiled parent keeps its output when its props do not change, so `rerender` with the same props does not reach a child: let a mocked hook subscribe with `useSyncExternalStore` and notify it inside `act` (see the thread query mock in `src/hooks/ai-chat-controller.browser.test.tsx`).
