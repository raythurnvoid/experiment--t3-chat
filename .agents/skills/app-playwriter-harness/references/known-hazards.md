...
- **A relocated popup can leave a stale Playwriter handle.** A Files browser popup appeared in `context.pages()`, but `snapshot` on the first session's popup handle timed out. Rebinding the exact existing popup URL through a second owned Playwriter session worked: locators read the live panel and clicked Resume normally. The popup still had the default `Vite + React + TS` title, which did not mean the app failed to load. Try a fresh session and exact-URL rebind before opening a replacement tab or testing synthetic transfer messages. Verified 2026-09-19. Do not close another task's tabs or reset the shared relay.
