...
- For long or assertion-heavy Playwriter flows, write the runner JavaScript to a file under `../t3-chat-+personal/+ai/` (the `-f` path is read by the CLI launcher, so a sibling-dir path is fine) and run `pnpx playwriter -s $session -f $scriptPath --timeout <ms>`; this avoids PowerShell quote corruption and makes reruns safer. Screenshot/output `path`s the page or CLI writes can also target `../t3-chat-+personal/+ai/`. Never put runner files or outputs in the repo's `tmp/` (it is tracked by git).
- If a temp Playwriter runner needs dynamic input it reads via `fs` from inside the sandbox, note the scoped filesystem is rooted at the repo and may not reach the sibling `../t3-chat-+personal/` path; only then fall back to the OS temp dir, read it by fixed path, and delete it when done. Do not assume PowerShell environment variables are available inside the Playwriter sandbox, and never use the repo's `tmp/`.
...
- For anonymous-vs-signed-in QA, prefer a fresh headless browser over mutating the user's real Edge auth state: `pnpx playwriter browser install` once, then `pnpx playwriter session new --browser headless`. The fresh profile mints a new anonymous app user on first load, the user's signed-in tabs stay untouched, and headless pages are foregrounded so screenshots, `waitForSelector`, and layout APIs all work (no backgrounded-tab limitations). Delete the session when done.
- Clerk sign-up is blocked in headless browsers: the Cloudflare Turnstile widget never mints a `cf-turnstile-response` token, so the Continue button stays stuck disabled in `cl-loading`. To get a signed-in throwaway user, skip the modal: create the user with the Clerk Backend API (`POST https://api.clerk.com/v1/users`; secret key is `CLERK_SECRET_KEY` in `convex env list`), mint a sign-in token (`POST /v1/sign_in_tokens`), then in the page run `Clerk.client.signIn.create({ strategy: "ticket", ticket })` + `Clerk.setActive({ session: res.createdSessionId })` and reload. The ticket strategy bypasses the captcha; the app bootstrap merges the session's anonymous user into the new account. Delete the user via `DELETE /v1/users/<id>` when done (owned rows such as publishers are not cascaded in dev).
- Close only the localhost tabs created for QA. Leave unrelated user tabs open.
- Edge's own `--remote-debugging-port=9222` no longer binds on the default user-data-dir (Chromium M136+ restriction; observed 2026-07-06: the flag is present on the process but nothing listens and `DevToolsActivePort` stays stale). Drive the user's Edge through the Playwriter extension relay instead of raw Edge CDP.
- On this Windows machine, multi-line code passed to `pnpx playwriter -e "..."` is truncated at the first newline — only the first statement runs, and it still reports `Code executed successfully`. Always use `-f <script.js>` for multi-statement code.
- Each Playwriter execute call has a ~10s sandbox cap. Keep one action per call (no long in-page sleeps or polling loops); poll durable state from outside via `convex data`/`convex run` readbacks between calls.
