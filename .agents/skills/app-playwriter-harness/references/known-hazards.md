...
- **A `context.route` handler outlives the runner that installed it, and the next run pays for it.** `page.route`/`context.route` live on the browser context, not on the call, so a runner that throws before its `unroute` leaves the handler in place. If that handler delays or blocks requests (a common trick for holding a loading state open long enough to measure), the NEXT run's `page.goto` silently waits on it and dies with `page.goto: Timeout 10000ms exceeded` — which reads like a slow app or a dead dev server and is neither. Open every runner with `await page.context().unrouteAll().catch(() => {});` and set `page.setDefaultNavigationTimeout(...)` explicitly, and put the `unroute` on the success path AND expect to lose it on the failure path. Observed 2026-09-04 while holding plugin asset responses to keep a startup cover on screen.
