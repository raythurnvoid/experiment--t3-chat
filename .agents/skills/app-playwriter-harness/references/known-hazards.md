...
- `files_nodes.archive_nodes` archives a whole subtree in one mutation. It costs about 9 ms of system time per item, so a folder with about 1,250 items fails with `too many system operations` (seen 2026-09-24). It also reads descendants that are already archived. So archiving the subfolders first does not help the parent: the top folder still failed with 3 active children and 1,248 archived ones. Clean up a big QA fixture by archiving its subfolders one at a time (400 items took about 11 s each). Keep big fixtures inside one throwaway parent you can leave in place.
