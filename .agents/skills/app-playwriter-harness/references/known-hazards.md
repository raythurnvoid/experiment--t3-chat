...
- `.MyTooltipContent` does set `pointer-events: none`, but the Ariakit portal wrapper around it does not, so a sidebar row's own metadata tooltip can cover that row's `More actions` button and make `click()` retry until it times out. Park the pointer away from the tree (`mouse.move(1400, 20)`), press `Escape`, wait ~600ms, then click. Do not force-click. The same wrapper blocks the sidebar search input after a keyboard chip removal, because focus lands on the previous chip's remove button and its tooltip opens over the input: `mouse.move` away and `input.focus()` instead of `click()` (2026-09-05).
...
- Ariakit tooltips do **not** open on a programmatic `locator.focus()` — the trigger takes focus, `[role="tooltip"]` never appears, and the check looks like a missing tooltip. They need a real pointer move (park away, then `mouse.move` onto the target, then nudge 3px, same rule as the hovercards above) or keyboard focus-visible. A `Tab` that moves focus onto the trigger is not always enough: after a mouse click in the tab, the trigger takes focus with no `data-focus-visible` and no tip, even when the next press lands a second later. What works: reach the trigger with `Tab`, then in a **separate** execute call press a harmless key (`keyboard.press("Shift")`) while it has focus. Ariakit then marks the focused element focus-visible and opens its tip. The same rule explains a late tip on the element you just left: a `Tab` pressed on a trigger that had focus but no focus-visible opens that trigger's tip as focus leaves it (seen 2026-09-24 on the Files header `Properties` button and breadcrumb crumbs). Dismiss them again before any document-scope axe run: the open portal is a bare `<div>` on `<body>`, outside every landmark, and it trips `region` on its own.
