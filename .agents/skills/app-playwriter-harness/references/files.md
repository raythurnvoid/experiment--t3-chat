...
- To resume into a new chat, send its first message and wait for the saved chat tab. An empty optimistic `New chat` tab does not yet enable `Resume agent`. For screenshot and tracing QA, use `expect(frame.locator(...))`, start `page.context().tracing` with screenshots, emit `page.screenshot()`, and stop tracing in `finally`. Open the stored result and then take control again; a native click must update the live image after the agent finishes.
...
- A model may repeat an earlier tool error without making a new call. Check for a new `Browser run` card and stored result. If it only repeats the old answer, use a fresh QA chat so the retry actually reaches the tool.
