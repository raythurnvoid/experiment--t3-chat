...
- Novel's `ImageResizer` never attaches to these embeds (it expects the selected node's DOM to be the `img` itself, not a wrapper span). The embeds own their controls instead (since 2026-08-08): clicking one selects it (`ProseMirror-selectednode` on the span) and, while real media is showing, a corner `.FileEditorRichTextMedia-resize-handle` (pointer drag commits `width`, double-click resets to natural size) and on images an `.FileEditorRichTextMedia-alt-button` appear. The alt button opens `.FileEditorRichTextMedia-alt-input` (Enter commits, Escape cancels, blur commits). A sized image serializes to markdown as a raw `<img src alt title width>` tag instead of `![...]`; commits re-select the node, so the controls stay up after each one.
