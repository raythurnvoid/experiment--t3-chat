...
- Plugin page host route: `/w/:organizationName/:workspaceName/plugins/:pluginName/pages/:pageId`. Root `.RoutePluginsPluginPage`; the plugin SPA runs in the sandboxed iframe `.RoutePluginsPluginPage-frame` (opaque origin — reach into it via `frame.contentDocument` evaluate, not app-document selectors). Loading state has `role="status"`; startup failure replaces the iframe with a `role="alert"` `.RoutePluginsPluginPage-error` containing a focused Retry button. Sidebar items for plugin pages come from `list_ui_pages` (pages with a `navItem`); see `references/plugin-gallery.md` for driving the Gallery page.
