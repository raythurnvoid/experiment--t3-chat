...
- The View dropdown replaces the header mode radios and file-view tabs. Locate its trigger with `getByRole("combobox", { name: /^View: / })`, its search input with `getByRole("combobox", { name: "Search views", exact: true })`, and its list with `getByRole("listbox", { name: "File views", exact: true })`. Choose a row with `getByRole("option", { name: "Markdown", exact: true })`. Check the resulting editor and route before typing.
- Plugin file views (Video Player and any plugin whose manifest declares a `fileViews` match for the content type) follow built-in options, named only by the view title, such as `getByRole("option", { name: "Video player", exact: true })`. They are never the default and have no `view=` query value. Selecting a plugin view mints the frame session and mounts `.PluginsUiFrame` inside `.FileNodeViewPluginView`. Observe until ready, then inspect the frame from `page.frames()` as for a plugin page. A stored video starts on File details with no viewer iframe. Switching away must unmount the frame and end its session.
- A file with `collaborationEnabled: false` (service uploads like Council `/meetings/<id>/transcript.md` and `summary.md`) supports every view its document shape supports. `files_resolve_effective_editor_view` (`packages/app/src/lib/files.ts`) takes only `requestedView` and `rootKind`; its one clamp is `plain_text` shape + `rich_text_editor` request. A non-collaborative `rich_text` file opens Rich text in `FileEditorRichTextNonCollab` and Review changes in `FileEditorDiffNonCollab`.
- Scroll owner: `.FileNodeView-editor-area`.
- Toolbar: `.FileNodeViewToolbar` scrolls horizontally and reserves scrollbar space. `.FileNodeViewToolbar-surface` paints only the 44px button row and its 1px bottom border. The native scrollbar track uses the same background to cover text scrolling underneath. With no scrollbar, the reserved space stays transparent. Check wide and narrow panels, horizontal scrolling, and text passing under the toolbar; the toolbar height must stay the same.
- Native scrollbars can be absent from Playwriter screenshots even when the toolbar overflows and scrolling works. Check `inspectElement` for `canScrollX`, the reserved height, and computed `scrollbarColor`. A capture without the native bar cannot prove its visible track is painted correctly.
...
- Check built-in option order against the stored shape: rich text has Rich text, Markdown, Review changes, File details; plain text has Code, Review changes, File details; editable HTML adds Preview before File details. Stored files offer File details. Matching plugin titles follow in installation order. Keep duplicate titles as separate rows with separate values.
- Check Folder and Home with an editable README of each shape. Offer only its editor options. A folder with no editable README has no picker. Renaming a plain-text file to `.md` must keep Code; renaming stored bytes to `.html` must not add Preview.
- The picker stays before Download and stays available in every file view, even with only one option. The popup is a dialog named `File views`. It contains one search field and one flat list of names. Check that it has no icons, provider names, groups, badges, or extra actions.
- Opening focuses Search views. Search trims whitespace and matches part of a name, ignoring case. A miss shows `No views found`. Filtering out the selected row must not change the current view or its trigger label. Clearing search restores every option; closing and reopening starts with empty search.
- Mouse click and Enter select and close. Opening, typing, hovering, arrows, and Escape must leave the active view and plugin-session count unchanged. Test closed-trigger ArrowUp, ArrowDown, and letter keys too, including after closing a filtered list. Escape and selection return focus to the trigger. An outside click keeps normal browser focus.
- Keep a local Markdown or Code draft, then visit File details, Preview when available, and a plugin view. Return to the same editor mode and read exact model text and undo history. The editor must remain mounted, hidden and inert while inactive. A late plugin response must not replace it. Plugin and preview frames must unmount when inactive.
- Check Review changes and the separate pending Review shortcut from every alternate view, including when the URL already says `diff_editor`. Automatic editor-mode changes must keep the alternate view selected. Opening another node or membership resets local selection. Removing the active plugin should restore the default with a notice; focus recovery must not interrupt another focused control.
- Check keyboard access, focus outlines, narrow width, real 200% browser zoom, and reduced motion. The popup must clear toolbar clipping and status controls. Check its cool gradients, selected and active rows, text contrast, and long names. The other sidebar tabs remain in place.

## HTML Preview And Meeting Brief Demo

- Editable files open in their effective editor mode; stored files open on File details. HTML text adds Preview to the View dropdown. Preview and plugin selections stay local while editor modes use the existing URL values. Stored content type and text shape decide the options; renaming an old text or blob file to `.html` does not convert it.
- Start the built runtime with the commands in `packages/file-preview/README.md`. Reuse Press on port 5173 and the preview runtime on port 5175. Development uses the local runtime by default; production needs `VITE_FILE_PREVIEW_URL` and the runtime's exact parent allowlist.
- Upload `assets/files/qa-meeting-brief.html` through the Files UI. Open it after processing finishes. Check the Code, Review changes, Preview, and File details options. Choose Preview, wait for “Chart ready”, click “Mark reviewed”, and check “Reviewed 1 time”. The chart uses a real pinned esm.sh import.
- In Code, make a local edit, then immediately select Preview. It should choose Editor draft. Return to Code and check the text and undo history. Save remains a separate action. Preview hides editor actions but keeps the View picker, Download, and pending panel available.
- A current agent proposal should be visible through Proposed changes before Accept or Save. Refresh reloads only the selected source. Changes while a preview runs show Updates available. A removed or stale source stops the frame and asks for another source or Review/Sync; it must not quietly show Saved. Review changes opens the Diff editor even when the URL already has `view=diff_editor`.
- Keep the runtime frame and its inner document separate. The inner frame's URL can appear as `about:srcdoc` or an empty string in Playwriter. Use `page.frameLocator('iframe[title^="HTML preview:"]').frameLocator('iframe[title="HTML preview document"]')` for stable chart and button checks. Parent-page DOM tests alone do not prove the preview ran.
- Native input worked in the HTML Code editor: locate the textbox named `File content editor`, press `Control+End`, then use `page.keyboard.insertText(...)`. Read the model through `window.__qa.monaco().plainText.getModel().getValue()` to prove the edit landed. `Control+z` and `Control+y` also worked across a Preview round trip. Older Monaco recipes below describe failures in other editor surfaces; check native input before using a model helper.
- HTML uses Monaco tokenization only, like CSS and JSON. If HTML logs missing `findDocumentLinks`, `findDocumentSymbols`, or `getFoldingRanges` worker handlers, check `app-monaco-config.ts`: all HTML language-service mode flags must stay disabled with the generic worker.
- Dropdown arrows move the active row only; Enter selects the view. Check keyboard entry into the frame and focus after Retry or a plugin option disappears. Check narrow width with a pending proposal: the status row must not cover the picker or popup. Preview should unmount its frames when another view or file opens.
- To check a collaboration toggle, open Properties while Saved Preview runs. The collaboration checkbox uses a 1px native input inside `.FilesPropertiesModalCollaboration-checkbox`; click that visible label, then the confirmation button. After the toggle finishes, the frame must be gone until Refresh. Test only an owned QA file because turning collaboration off removes its shared edit history.
- Review controls live outside Monaco's DOM. With a proposed HTML file open, check that hunk buttons sit beside their lines and never cover the View picker or popup. Leaving the editor must remove the hunk buttons. The widget uses the editor's CSS anchor and Monaco's relative line offset; see the files-editors skill before changing its position.
- From Refresh, a native Tab should enter the document's first control. Check that Enter activates it inside the inner frame. For a narrow view, open and close Source, then check the inner document's scroll width as well as the host controls.

For the meeting demo, create `assets/files/agent-skills/.agents/skills/meeting-brief/SKILL.md` at `/.agents/skills/meeting-brief/SKILL.md` in the app workspace. Enable workspace instructions for catalog discovery. Ask the agent to use meeting-brief, read the chosen Council transcript and summary, and draft the follow-up email plus HTML brief. The agent reads the skill with Bash and sees its own pending edits. Check source-grounded facts, preview the proposed HTML, request one focused revision, Refresh, then review and save. Email remains a draft.

Verify successful file-tool results before looking for the outputs. If a turn reaches its step limit while finding sources, continue the same chat with the known paths. A message claiming that files were created is not write evidence. When opening a path from Pending changes, scope the exact path text to `.FileEditorSidebarPending`; the same path may also appear in hidden chat content.
...
Diff view with a pending proposal (since 2026-09-06, `FileEditorDiff` in its collaboration-off mode): when the signed-in member has an agent proposal on the file, `view=diff_editor` reviews it instead.
...
- `Accept all` and `Discard all` act only on the currently shown rows and on the hidden folder drafts that hold them. `Accept all` saves each shown draft's pending parent folders first. `Discard all` removes a hidden folder only when a shown draft inside it is discarded too. Their accessible names are `Accept all shown pending changes` and `Discard all shown pending changes`; both are disabled when the selected source has no rows. If accepting a shown row would also settle or invalidate a hidden row, the app asks the user to switch to `All changes`.
...
6. On disposable data, run a bulk action from one chat view. Confirm only its shown rows and their hidden parent folder drafts settle; rows that belong only to another chat or `Your edits` remain. A shared row settles for every source because it is one pending doc.
