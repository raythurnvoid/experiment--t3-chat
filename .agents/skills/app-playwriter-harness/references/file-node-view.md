...
For the meeting demo, save `assets/files/agent-skills/.agents/skills/meeting-brief/SKILL.md` at `/.agents/skills/meeting-brief/SKILL.md` in the app workspace. The workspace instructions feature must be enabled. Accept and Save the skill before the next agent message. Select it in Instructions and skills, then ask the agent to read the chosen Council transcript and summary and draft the follow-up email plus HTML brief. Check source-grounded facts, preview the proposed HTML, request one focused revision, Refresh, then review and save. Email remains a draft.
