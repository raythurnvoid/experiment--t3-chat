...
- File tabs and editor modes are separate. Editable files open on Editor, stored files on File details. HTML text adds Preview before matching plugins. A file's stored content type and text shape decide this; renaming an old text or blob file to `.html` does not convert it.
...
- Upload `assets/files/qa-meeting-brief.html` through the Files UI. Open it after processing finishes. Check Code and the Editor/Preview tabs. In Preview, wait for “Chart ready”, click “Mark reviewed”, and check “Reviewed 1 time”. The chart uses a real pinned esm.sh import.
- In Code, make a local edit, then immediately select Preview. It should choose Editor draft. Return to Editor and check the text and undo history. Save remains a separate action. Preview hides the editor toolbar and mode controls but keeps Download and the pending panel available.
- A current agent proposal should be visible through Proposed changes before Accept or Save. Refresh reloads only the selected source. Changes while a preview runs show Updates available. A removed or stale source stops the frame and asks for another source or Review/Sync; it must not quietly show Saved. Review changes returns to Editor even when the URL already has `view=diff_editor`.
...
- Tab arrows move focus only; Enter or Space activates the focused view. Check keyboard entry into the frame and focus after Retry or a plugin tab disappears. Check narrow width with a pending proposal: the status row must not cover the tabs. Preview should unmount its frames when another tab or file opens.
...
- Review controls live outside Monaco's DOM. With a proposed HTML file open, check that hunk buttons sit beside their lines and never cover Editor or Preview. Hiding Editor must remove the hunk buttons. The widget uses the editor's CSS anchor and Monaco's relative line offset; see the files-editors skill before changing its position.
