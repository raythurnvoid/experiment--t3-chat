...
- Toolbar: `.FileNodeViewToolbar` scrolls horizontally and reserves scrollbar space. `.FileNodeViewToolbar-surface` paints only the 44px button row and its 1px bottom border; the gutter below stays transparent. Check both a wide panel with no overflow and a narrow panel with a working scrollbar.
