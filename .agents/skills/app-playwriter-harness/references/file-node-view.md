...
Diff view with a pending proposal (since 2026-09-06, `FileEditorDiff` in its collaboration-off mode): when the signed-in member has an agent proposal on the file, `view=diff_editor` reviews it instead. The root is `.FileEditorDiff` (not `.FileEditorDiffNonCollab`), the original pane is the staged branch (the committed text, re-serialized for a Markdown file, until a hunk is accepted) and the modified pane is the unstaged branch (the proposal), the hunk widgets and `Save staged changes` / `Accept all` / `Discard all` work as for a collaborative file, and the toolbar has no `Sync with live file` and no `Open file snapshots`. Save shows the toast `Changes saved` and the view exits to the default editor once the doc is gone. A stale proposal (a member saved the file after the agent made it) renders read-only: a `role="status"` line `.FileEditorDiff-stale` carries the stale sentence, `editor.getOption(monaco.editor.EditorOption.readOnly)` is true on `diffModified`, there are no hunk widgets, and the toolbar holds only `getByRole("button", { name: "Discard proposal" })`. While the pending list is still loading the view shows `FileEditorDiffSkeleton` (`role="status"`, sr-only `Loading changes…`). The pending row of a stale proposal shows the caption `Out of date`, its link name ends with `, out of date`, and its Accept button stays enabled with the sentence in `title`. Save keeps `Save staged changes` disabled until the view's doc query shows the save, which can land about a second after the action result, so assert on the toast and the exit rather than a fixed delay. `mv -f <src> <dst>` onto a file with collaboration off is a replace proposal, not a content one: the row caption is `Replaced`, its buttons are `Accept move of <src> to <dst>` / `Discard move of <src> to <dst>` (not `Accept changes to`), and Accept moves the source node onto the path with its own asset and archives the old node under the same id, so resolve the id again from `list_tree` afterwards. Collaboration changes use the preservation and preparation flow below. The `.FileEditorDiff-stale` status line is always rendered, so assert its text, not its presence. `list_files_pending_updates` rows hold the node under `fileNodeId`. To move between files without a reload (the chooser in `FileEditorInner` keeps state across nodes, which only an in-app move can exercise), navigate from page context: `const { app_router } = await import("/src/lib/app-router.ts"); await app_router().navigate({ to: "/w/$organizationName/$workspaceName/files", params: { organizationName: "personal", workspaceName: "home" }, search: (prev) => ({ ...prev, nodeId, view: "diff_editor" }) })`. Every such move between diff views logs `[pageerror] no diff result available`; that is Monaco's own (see `collab-yjs-comments-regression.md`), not an app fault.

Review exit checks:

- At a 1156px viewport with both sidebars open, check that stale and loading status text starts below the floating Pending bar. In the stale toolbar, Discard proposal must stay clear. The page must not overflow sideways. An empty status line must take no space.
- Accept all + save, content discard, and a failed branch reload return to the file's default editor with history replacement. Read `history.length` before and after exit; it must not grow. Then use Back and assert the empty `.FileEditorDiffNonCollab` view does not mount.
...
Fixture recipe: see "Non-Collaborative File Fixture" in `files.md`. For an existing fixture with accepted hunks, use the same-text save and stale-discard reset in `known-hazards.md`. Resolve active nodes by id or skip `archiveOperationId` when finding them by path.
...
2. Record the proposal id, `updatedAt`, three state ids, branch texts, and exact committed text. Use fresh authenticated HTTP queries for `get_file_pending_update` and `list_files_pending_updates`; the React client's subscription cache can return the state before a write. Fetch all three branches with `files_fetch_file_pending_update_yjs_state` and decode them with the node's `yjsRootKind`. Use a signed download for the saved text.
...
4. Before opening Review, read the stored proposal again. Its id, old branch ids, text, and `updatedAt` must be unchanged, with `contentNeedsRebase: true`. `get_file_pending_update` also returns `currentYjsLastSequenceId` from the current file. That field is not stored on the proposal and changes across the switch. The Pending count must still include it and the row must say `Review after collaboration change`.
5. Try single and bulk content Accept before Review. They must keep marked content pending and explain why it needs review. Ordinary agent reads must return committed text; an agent write must refuse without replacing the old proposal. Run the Bash read and refused write separately so a failed write cannot hide the read output. A pending delete keeps its own action. A marked content-plus-move row's combined Accept still refuses. To test its separate move, use Diff's `Discard proposal` to remove only content, then accept the remaining move-only row. The Pending row's Discard removes both parts.
...
9. Before toggling, save a newer edit far from the proposal's edit. Switch modes and open Review; both edits must survive. Repeat with different edits in the same place. A conflict must leave the old proposal readable and committed text unchanged. Check `Copy accepted text`, `Copy proposed text`, Retry, and Discard. Windows clipboard reads can use CRLF; compare against the expected text after normalizing line endings. Copy both proposal texts, resolve the file in its normal editor, and discard the old proposal; saving one branch and retrying may still conflict with the other.
