...
2. Record the proposal id, `updatedAt`, three state ids, branch texts, and exact committed text. Use fresh authenticated HTTP queries for `get_file_pending_update` and `list_files_pending_updates`; the React client's subscription cache can return the state before a write. Fetch all three branches with `files_fetch_file_pending_update_yjs_state` and decode them with `contentRebaseRootKind ?? node.yjsRootKind`. Use a signed download for saved non-collaborative text. `files_fetch_file_yjs_state_and_text` returns null for those files; for collaborative files its `text` is a Result, so read `text._yay` after checking `_nay`.
