...
- The verdict call is `generateText` with `Output.object`, and `generate_verdict` returns `result.output`. Reading `output` throws when the model wrote nothing the schema accepts, and also when it stopped before finishing. The publish action catches that throw and refuses to register the version, so the gate stays closed. Do not replace that read with anything that can return `undefined` instead of throwing: an undefined verdict would flow into `upsert_version_review` and the gate would fail open.
...
- Canonical asset responses retain CSP, `nosniff`, immutable cache, and CORP. Their CSP uses `'self'` for scripts, styles, fonts, API connections, and same-origin module/style fetches. R2 failures return 502 with `Cache-Control: no-store` + `Retry-After: 3`. Never log raw R2 errors or keys.
