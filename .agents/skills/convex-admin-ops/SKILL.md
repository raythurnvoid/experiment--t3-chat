...
Tenant purge drains each producer's Activity and dismissal docs. Account finalization drains the user's dismissals and notifications. Notifications in another recipient's inbox that only name the deleted actor remain by design. See [data-deletion](../data-deletion/SKILL.md#workspace-content-purge-coverage) for the full cleanup scope.
