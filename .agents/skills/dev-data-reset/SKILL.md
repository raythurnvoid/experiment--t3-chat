...
4. **Orphan sweep (conditional).** Plugin publishes materialize source files under the GLOBAL organization / GITHUB workspace; neither step 1 nor step 2 touches them. Count `files_nodes` for that tenant — if nonzero, sweep with a temporary batch internalMutation modeled on `github_sources.delete_mount_content_batch` (chunks → file_stats → metadata → R2 asset + node), run until `done: true`, then delete the temp module and redeploy.
