...
Chitchat has a separate database. The local account tombstone also advances its membership lifetimes and saves removal events in the same Press mutation. Tenant deletion and the first workspace purge save terminal events; automatic ownership transfer saves a permission refresh. These operations do not wait for the remote backend. Its remaining access is bounded by a 30-second lease. Keep lifetime and connection markers so ordered removal, later recovery, and terminal event delivery remain possible. See [auth-system](../auth-system/SKILL.md#chitchat-in-its-own-backend).
...
- If the user doc is already gone, keep the request through the same bounded user-owned drain order: plugin UI sessions, publisher docs, notifications, direct grants, then `db_drain_user_finalization_batch`. Membership draining still queues an empty personal organization before deleting its last membership. After those families are empty, delete any remaining anonymous-token and billing-snapshot docs left by an older tombstone purge. Then remove the stale request.
- A non-tombstoned user request should make no destructive progress and should log.
- Before the small final transaction it drains one indexed family per pass. The order is plugin UI sessions, publisher docs, notifications, direct grants, transfer runs, background Bash jobs and their note cursors, plugin service grants, memberships, role assignments, draft expiry checks, pending-update children and parents, last-sequence docs, pending Yjs pages and states, pending text inputs, operation batches, trusted stages, deleted-append replay receipts, plugin member usage, quota docs, API credentials, and public API grants. The admin path drains transfer runs and Bash jobs before plugin UI sessions. Receipt deletion returns the installation tombstone slot and the exact old member-counter slot before that counter doc is removed. The queued path returns `done: false` after each non-empty batch, so its request doc stays in place.
...
- `files_write_policy_runs` (protection jobs) right after review runs, one job per pass through `files_write_policy_runs_db_delete_run_batch`. It stops the job first, so a scheduled step writes nothing.
- `files_archive_runs` (archive and restore jobs) right after protection jobs, one job per pass through `files_archive_runs_db_delete_run_batch`. It stops the job first, so a scheduled step writes nothing. Stopping a running restore job starts the user's next queued one, and a later pass stops and deletes that one too.
...
- `ai_chat_threads_messages_aisdk_5` (the `aisdk_5` in that name is stored data and does not track the AI SDK major version), `ai_chat_bash_shell_transcripts`, `ai_chat_bash_shells`, `ai_chat_bash_job_notice_cursors` (per user and thread; there is no per-thread delete, so they die with the thread here), `ai_chat_threads`
- `api_credentials`
- `public_api_grants`
- External file writer cleanup drains `plugins_external_file_reader_changes` before the matching
  operation receipts, bindings, writers, and service-grant request receipts. These are bounded
  workspace passes after the plugin fence. Ordinary uninstall preserves the markers and Files grants.
...
- Enqueue durable exact-key deletion jobs before deleting the Convex doc that stores the object key.
...
- Every growing user-finalization family uses an indexed `.take(batchSize)` pass, except the chosen successor's organization roles: those are bounded to six by the workspace limit and stay atomic with ownership transfer. Pending-update expiry checks are deleted first. Exact chunks, plain chunks, and metadata are deleted before their pending-update parent. Pending Yjs pages are deleted before their state parent. The one-row anonymous-token, billing-snapshot, and `users_last_active` invariants stay in the small final transaction. An early tombstone purge and the missing-user request branch also delete `users_last_active`, and a heartbeat from a deleted user's open tab does not write it again.
