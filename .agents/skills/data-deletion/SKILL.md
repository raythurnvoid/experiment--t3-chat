...
- `files_archive_runs` (archive and restore jobs) right after protection jobs, one job per pass through `files_archive_runs_db_delete_run_batch`. It stops the job first, so a scheduled step writes nothing.
