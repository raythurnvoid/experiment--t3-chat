...
- `db_drain_user_direct_permission_grants_batch`: deletes one bounded indexed batch of every direct user grant, including file and plugin-scope grants. It schedules the existing stranded-scope cleanup for the plugin scopes named by that batch, so private projection ACLs follow the surviving principals. Both user-deletion paths drain this index to zero before finalization.
...
- Sets `pluginDataPurgeStartedAt` on every retained workspace doc before returning, so plugin
  sessions, services, runs, store calls, and scheduled projections stop during retention.
...
- The plugin document store goes through `plugins_data_db_drain_batch` with `installationId: null`, which covers every installation in the workspace. The durable workspace fence makes `plugins.install_version` refuse new installs and re-enables. Every central plugin UI, store, service, runtime, and projection gate also requires the workspace doc to exist with no fence. The later disabled status is a second guard while live scope rows and released fences drain. Both guards stay until the later session and installation passes remove the records. A data-only reset clears the fence only when the full reset finishes; a deleted workspace removes it with the workspace doc, and the missing doc itself keeps the gates closed. The store helper deletes projection rows child-first: dirty channels; Chitchat staged items, reactions, author labels, and files; Chitchat builds; projection file maps; then projection states. It reads at most one staged file body per pass. Reservations, deleted-append replay receipts, revision tombstones, documents, service grants, `plugin_scope` grants, live scope rows, and scope lifecycle rows (identity markers plus real released-range fences) follow. It then drains service destination fences and `plugin_service_storage_targets` only in workspace mode, member usage, and the accounting doc. The accounting doc goes last so it is never the survivor. It runs before the installation pass, so no row points at a deleted installation. An installation-scoped drain writes to no `files_nodes` row and deletes no service destination fence, storage target, or mirrored private-folder `file` grant: uploaded and projected files belong to the workspace and outlive the plugin. A `plugin_scope` grant whose `resourceId` is `"<installationId>:<scopeId>"` does leave with its live store row; a `file` grant naming a mapped folder stays because the file stays. The derived `/chitchat/` files remain as a frozen snapshot. A placeholder whose upload never finished stays as an empty file a member can delete.
